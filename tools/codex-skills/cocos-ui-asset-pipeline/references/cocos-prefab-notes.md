...
- 生成式透明 UI 可能带有肉眼几乎不可见的低 alpha 尾迹，普通 `trim` 仍会把尾迹计入边界，导致可见主体在画布内偏上或偏下。接入前除原始 alpha 外，再以约 0.5%～2% alpha 阈值测量可见包围盒，并把可见主体重新居中到固定画布；若底板主体偏心，应先修正底板素材并保留 `.meta`，不要给每个图标和 Label 分别添加反向偏移。
