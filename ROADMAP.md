...
本文保留早期原型技术路线。下方“当前定位”已在 2026-09-22 校正；其后旧技术清单是历史记录，不能直接用作现行宣传或参数依据。最新娱乐版本、素材归属及录制清单见 [适龄调整计划第 22 节](docs/娱乐模式8加适龄调整计划.zh.md#22-g-全局表达与提交素材统一2026-09-22)。内容以代码为准，尤其是 `assets/scripts/core/GameManager.ts`、`assets/scripts/venue/*`、`assets/scripts/entity/*` 和 `assets/scripts/core/ResourcePaths.ts`。

## 当前定位

《划水大师》（项目名 `Speed Swimming 3D`）是轻量 3D 游泳竞速游戏。玩家在屏幕左右半区点按或按住，自己选择划水侧与节奏；现有标准竞速、狂野模式和娱乐模式。娱乐模式从七种事件中按赛程轮换，包含游戏补给、定时水球、充气玩具鲨、漩涡、水球点名、喷水浮标与杂物漂流。

现行快速比赛可选 200 米／400 米，通过 50 米泳池折返完成。微信好友房间使用现有联机实现；抖音联机尚未接入，不能作双平台联机宣传。巨浪仅为独立调试内容，未加入正式七事件池。S1 道具仍影响游戏心率，整体适龄与平台审核尚待核验。
