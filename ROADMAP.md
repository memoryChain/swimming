...
- 运行时泳池入口集中在 `RESOURCE_PATHS.poolPrefab = 'pool/PoolScene'`，也就是 `assets/resources/pool/PoolScene.prefab`。
- 泳馆资源应以 `tools/LowPolySwimVenue.blend` 导出的内容为准。不要在运行时代码里临时拼出另一套泳池，也不要把旧的 `LowPolyPool` 版本和新泳馆叠加。
- Blender 源文件、预览图、备份文件只保留在 `tools/`，不放进 `assets/` 发布资源目录。
...
2. `VenueManager` 加载 `PoolScene.prefab`，并绑定水面材质。
3. 泳池加载完成后一帧，`RaceCourseLayout.calibrateFromPoolScene()` 根据泳池节点校准赛道。
4. 观众面片在泳池加载和赛道校准之后创建。
5. 玩家、AI、UI、`RaceManager`、输入路由和流程控制器最后创建。
...
- 观众数量通过 `SPECTATOR_DENSITY_SCALE = 0.5` 控制，当前约为之前密度的一半。
...
- `CartoonSwimmerRig.ts` 使用统一常量 `SWIMMER_MODEL_SCALE = 1.5`，比赛、赛前站立、完赛漂浮和模型调试都使用同一个缩放值。
- 角色 prefab 候选路径集中在 `ResourcePaths.ts`，默认仍是 `models/UserSwimmerLow`。
- 运行时换色、描边、完美输入闪光、水花和手动自由泳姿态都由角色模块负责。
...
- 比赛 HUD 和 debug model HUD 会显示稳定性、加速度、速度等反馈。
...
- 保持 Blender 源文件在 `tools/`，运行时只加载 `assets/resources` 下的 prefab、glb、材质和 effect。
- 后续导出泳馆时，以 `LowPolySwimVenue.blend` 为源，导出后检查 `PoolScene.prefab` 中 marker、水面、看台行和跳台节点名是否仍可被代码识别。
...
- 主场景从 `PoolScene.prefab` 加载泳馆，不叠加旧泳池，不额外生成无关场馆物件。
