...
test('五种荣誉状态；未完成、退出和淘汰不授予前三名奖牌', () => {
        ...
        assert.equal(settlementTime({ time: 0, eliminated: true, sharkEliminated: true }), '碰撞退赛');
        assert.equal(settlementTime({ time: 0, eliminated: true, cannonEliminated: true }), '水球出局');
        const v = make();
