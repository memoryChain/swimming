...
const method=cls.members.find(n=>n.name?.getText(source)==='setHallOffset');
...
test('大厅角色用取景平移维持位置，相机始终使用完整视口，不污染后续弹窗',()=>{
    ...
    const Preview=vm.runInNewContext(ts.transpileModule(`class Preview { ${method.getText(source)} };Preview`,
