...
function fixture(kind='buoy', count=2) {
    ...
    const loader={loadSwimmerPrefab:cb=>pending.push(cb),setLayerRecursive:()=>{},findNode:(n,name)=>n.name===name?n:n.children.map(c=>loader.findNode(c,name)).find(Boolean)||null};
    const core={};
    ...
    Object.assign(h.cc,{Node,Material,MeshRenderer,Color,gfx:{CullMode:{NONE:0}},
        ...
        primitives:{sphere},
        instantiate:()=>{const imported=new Node('imported');for(const name of kind==='buoy'?['BuoyBody','BuoyBalloon']:['CannonBase','CannonNozzle']){const n=new Node(name);n.setParent(imported);n.addComponent(MeshRenderer).mesh={imported:true,name}}return imported},
    });
    ...
    return {...h,Node,p,world,course,pool,states,nodes,meshes,materials,pending,splashes,
        ready:(error=null)=>pending.shift()(error,error?null:{prefab:{}}),
