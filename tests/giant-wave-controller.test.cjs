...
test('控制器只创建一套表现，AI追浪、HUD、重开和销毁正确清理', () => {
    ...
    const autopilot = { target: null, setGiantWaveTargetZ(value) { this.target = value; } };
    const replacement = { target: null, setGiantWaveTargetZ(value) { this.target = value; } };
    c.update(.2, true, autopilot);
    swimmers[0].startPosition.z = c.simulation.state.z;
    c.update(.2, true, autopilot);
    assert.notEqual(autopilot.target, null, '托管选手收到本波目标');
    c.update(.2, true, null);
    assert.equal(autopilot.target, null, '关闭托管立即清除旧目标');
    c.update(.2, true, autopilot);
    c.update(.2, true, replacement);
    assert.equal(autopilot.target, null, '替换托管控制器不遗留旧目标');
    assert.notEqual(replacement.target, null);
    c.reset();
    assert.equal(replacement.target, null, '直接重开也清除托管目标');
    c.update(.2, true, autopilot);
    assert.notEqual(autopilot.target, null);
    c.dispose();
