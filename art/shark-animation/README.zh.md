...
- [自包含离线预览](review.html)：八个角度／图标加三个咬合关键姿态切换，内嵌所有图像与当前 GLB 下载，无外部依赖；为静态作者源预览，不是实机录像。
...
最终 GLB 为 **119,008 字节、2,648 三角面、1,408 导出顶点、1 蒙皮网格、1 不透明顶点色材质、0 贴图、7 骨骼**。原 GLB 为 248,976 字节、2,813 三角面。实际读取新旧逆绑定矩阵，最大差值为零；原下颌骨已恢复蒙皮。新同源静态回退为 503 顶点／846 三角面，加载完成后释放，不额外常驻。
...
python scripts/run-blender.py -- --python art/shark-animation/render_shark_review.py -- --apply-icon --bite-poses
# 同源回退，不运行逐帧动作采样。
...
命中复用 B1 浮圈、入场复用 B2 重入水；去重覆盖模型晚加载、快照乱序、可靠事件尾段和旧局延迟水花。退出只下沉鲨鱼外观层，角色短咬后立即脱离，不被持续夹住或拖走。接触／入场显式采样限 24Hz，隐藏状态提前返回，变换只在变化时写；没有新逐帧字符串、Graphics 绘制、临时向量、透明外壳或粒子池。

两条路径复用现有 256×144／最高 30Hz 事件画中画，FOV 48°，看清玩具、选手及绕行方向；没有新增相机、RenderTexture 或改变优先级。二十组广播、栏目“玩具开咬”、模式入口、提示、结束语与图标已统一。实际代码及文件审计未发现鲨鱼独立咬合／惨叫音效接线，因此没有虚构音频替换或实机试听结论。
...
当前 GLB 119008 字节、2648 三角面、7 骨骼，1 网格／1 材质／0 贴图；旧版修长顶推源保存在 archive/SharkModel_toy_push.blend 与对应脚本、GLB。实际 GLB 中下颌旋转已验证 26° 张开、接触点闭合和 12° 松口，游动／咬合通道首尾零偏差，逆绑定矩阵与原骨架零偏差。当前 meta 记录 2648 三角面，与实际几何一致。

新预览有 11 个角度／姿态按钮，已在独立浏览器打开，图像加载与切换正常，无外部请求或横向溢出。没有恢复逐帧采样。静态回退只保证加载前／失败时仍可识别，完整下颌动画由正式 GLB 提供。

本轮最终自动检查：娱乐聚合 **252／252**、鲨鱼专项 **66／66**、事件镜头 **11／11**，TypeScript 5.4.5 指定检查通过。字体已重新生成并审计，**1,579 字符／280 个工程文案文件**；纹理 fix／check 通过，扫描 **282 项**。这些结果包含 F 已完成的最新共用表达，未回退其补给、漩涡和杂物改动。
