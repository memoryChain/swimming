...
- 纹理／导入：此前 Creator 现有会话已自动更新三段动作和顶点色材质，原主 UUID、原两段动作子 UUID 保留，旧贴图子资源删除。**最终修长版导出后，当前 meta 仍记录前版 2,452 三角面，而实际 GLB 为 2,644；最终改形需要在用户的 Creator 会话重新导入。** fix／check 通过当前元数据政策检查，不能代替最终模型导入。未启动、重启或截图 Creator。
