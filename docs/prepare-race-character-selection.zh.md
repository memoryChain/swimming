...
弹框显示在主 HUD 层，打开时暂停 3D 角色预览相机，因此说明卡显示与点击使用同一 UI 坐标系；它靠近点击行并限制在可见画布内。点击空白或右上角关闭，点击面板只消费触摸；无自动消失计时器，避免说明尚未读完即消失。流程内只按需创建一个实例，切页、离场、切换属性／外观页签或角色时收起并恢复预览，流程销毁时解绑窗口监听并销毁弹框。窗口尺寸变化时收起，下一次点击按新尺寸定位；重复开关不重建节点、重复加载素材或重启预览。
...
属性 tips 定位仍保留通用相机桥接：不能假设锚点和说明卡永远属于同一 Canvas，点击时先用属性行所属 Canvas 相机做 `worldToScreen`，再用目标 HUD 相机做 `screenToWorld`，最后转入弹框局部空间。当前主界面两者通常是同一相机，但这层转换可防止未来跨 Canvas 复用再次错位。三项说明继续贴近所点属性区并保留画布边界限制；缺少相机时不显示错误位置。回归仍使用不同画布原点、屏幕缩放和三种横屏宽度验证转换，不以两个原点均为零的替身代替。
