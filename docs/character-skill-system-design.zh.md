...
# 四角色专属大招：当前第一版定稿（2026-08）

> 本节优先级高于下方早期“爆发冲刺原型”和旧四角色草案；旧内容仅保留作设计演变记录。

- 共享规则：全员共用 0–100 大招能量；海豚跃消耗 30；满 100 才能在正常水面点击右下圆形按钮释放，释放清空能量。转身、水下、海豚跃期间不能释放；技能激活后进入这些阶段不会暂停、延长或补发。
- 铁臂狂鲨「鲨尾重击」：瞬发速度冲量，使用厚重水花表示。没有持续环。
- 灵波飞鱼「律动水线」：5 秒内的 3 次有效划水，GOOD 按 PERFECT 结算，PERFECT 也消耗一次；MISS 不消耗。HUD 显示剩余次数和过期环。
- 破浪新星「踏浪追击」：2 秒三段自动推进，分别在 0、约 0.65、约 1.3 秒触发；转身/水下/海豚跃时对应段直接跳过，不暂停也不补发。HUD 显示倒计时环和三段标记。
- 深海潜将「深海航迹」：5 秒内仅正常水面游泳降低阻力；特殊阶段照常走秒，但不继续减阻。
- 联机：`UltimateActivate` 仍走可靠帧事件；角色 ID 从既有 modifier digest 解析技能类型。self snapshot 额外携带量化剩余时长、次数和已触发脉冲数，用于丢帧后收敛。AI 保留蓄气和海豚跃，但不释放大招。
- 平衡目标：200m、相近操作下，单次大招平均约带来 2m 收益；所有数值均通过 `tuning.json` 调整，以实测为准。
