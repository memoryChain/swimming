...
> 2026-09-26 复核：当前仍全部使用本地 MockBackend。完整云档案、RankManager 能力边界、后台改档、迁移与费用以[最新接入调研](微信得分存档与玩家云档案接入调研.zh.md)为准；本文其余内容保留为早期架构规划。
...
- 「换手机养成不能丢」= 养成数据必须存**云端、按微信账号（openid）存**。所以**需要一个后端**——但它是**免传统服务器运维、基础套餐加超额按量计费**的 Serverless，不是传统游戏服务器。
...
| 费用 | 当前基础套餐 19.9 元/月/环境，超额另计；以官方控制台为准 | 固定月租 |
...
- 玩法 ID 在 MP 后台配，类型有**数值型 / 时间型 / 枚举型**。**游泳成绩可考虑「时间型」，上报单位、排序与最佳值语义需在 MP 配置及真机验证**。
- **适合**：排行榜、最快成绩、历史最高分、简单进度（如最高关卡）。这些**可通过平台跨设备查询、免自建服务器**；全服榜单查询能力另行确认。
