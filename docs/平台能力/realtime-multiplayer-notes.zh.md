...
- **2026-09-21 玩法版本94（当前）**：垃圾恢复三种瓶型与泡沫餐盒、每波六件按2＋2＋2入水、18槽、16秒漂浮及5.5秒下沉；分组由既有spawnOrder及年龄恢复。房主独占生成与接触，保留v90～93的可靠事件去重、迟到补偿、迁移和raceId外壳。`L|`每槽21个量化整数改36进制，精度与字段数不变；含外壳的18槽标准压力包1300字节，全负横坐标及六位接触修订压力包1390字节。双方须更新到v94，v93及更旧客户端不能混跑。真机性能与弱网验收仍待完成。
