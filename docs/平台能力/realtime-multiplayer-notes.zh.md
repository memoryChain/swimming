...
- **2026-09-18 玩法版本 53（当前）**：新增 200 米水雷接力赛。全程单雷逐轮缩短引信，持雷者贴身自动传递，上一持有者有防回传保护，最后 0.8 秒锁定；爆炸只击飞失速，不淘汰，持雷冲线可拆弹。房主独占装雷、传递、爆炸／拆弹判定，可靠 `m`／`t`／`b` 事件发送持有关系与剩余引信；`S|` 快照追加活动轮次、持有者、保护计时、完成／爆炸掩码和每轮最终持有泳道压缩表，支持丢包恢复和房主迁移。52 及更旧版本不能混跑；离线回归覆盖传递保护、锁定、拆弹、确定性选择、事件与快照编解码，双端真机表现待验证。
