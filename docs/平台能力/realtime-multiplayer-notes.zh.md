...
- **2026-09-19 玩法版本 81（当前）**：房主的定时炸弹传递优先消费人物碰撞求解器的稳定接触记录，保证已经触发击退／翻滚的真实碰撞不会因求解器先把两人分开而漏传；无接触记录时继续使用身体椭圆、跨帧扫掠和定向短传。仍复用既有可靠传递事件与快照，不增加无线字段；版本 80 及更旧客户端不能混跑，避免同一次人物碰撞在不同规则版本上产生不同归属。
