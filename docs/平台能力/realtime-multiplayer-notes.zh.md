...
- **condition 权威**：真人的体力比例/心率随 frame self（广播降级时随 `P|`/`IN|`）由 owner 提供，真正 AI 由房主 `S|` 提供。心率在网络上向下取整，确保 110/150/175 分区边界不会因四舍五入跨区；AI 的体力耗尽冷却剩余也随 `S|` 量化到毫秒，房主迁移不能重新开启完整冷却。`P|` 在尾部携带 owner seat，用于拦截正常代码产生的 lane/seat 错配。
