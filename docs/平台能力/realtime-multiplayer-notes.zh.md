...
- **2026-09-20 玩法版本 85（当前）**：巡游鲨鱼的防卡死补咬不再直接击倒后补播动画，而是进入独立 `PATROL_BITE` 状态：先停顿蓄势 0.09 秒，在闭合节点通过既有可靠鲨鱼事件击倒并短扑，结束后回到巡游且不推进正式追猎轮次。状态继续使用鲨鱼快照原字段，动画按序号去重；访客漏收蓄势快照时由可靠击倒事件补播，房主迁移按剩余时间恢复。版本 84 及更旧客户端不能混跑。
