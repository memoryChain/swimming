...
- **AI 海豚大招是独立的权威动作边**：协议 v5 的 `AiDolphinJump` 携带按房主、按泳道单调递增的 `aiActionSeq`，同一动作的全部冗余包共用一个序号。接收端按 `(房主 seat, lane, aiActionSeq)` 永久去重，并在普通 packet/frame 顺序过滤之前处理，因此首包丢失、后续重复包首次到达或旧包迟到都只会执行一次。房主迁移后新 seat 使用独立序列；房间名单确认旧房主离开时，所有端立即改信任最低存活 seat，旧房主迟到包直接丢弃。任何房主都必须先发出自己的首个 `S|`，再获得 AI 动作决策权。
...
- **AI 海豚动作起点权威**：房主广播已接受的 AI 海豚动作时同时量化携带起始距离、侧移、速度、朝向及角速度、轴向翻滚、碰撞俯仰和两轴碰撞余势。远端使用专门的权威重放入口：先替换本地预测的翻身／下潜／旧海豚阶段并恢复房主起点，再进入共用轨迹初始化；不得重新检查本地剩余距离、姿态、能量或预测阶段。这样房主在阈值一侧、远端漂到另一侧时也不会否决已经成立的动作。
- **condition 权威**：真人的体力比例/心率与 `sprintActive` 随 frame self（广播降级时随 `P|`/`IN|`）由 owner 提供，真正 AI 由房主 `S|` 提供。远端真人用该状态重放冲刺推进倍率，不能只靠事后位置校正；心率在网络上向下取整，确保 110/150/175 分区边界不会因四舍五入跨区；AI 的体力耗尽冷却剩余也随 `S|` 量化到毫秒，房主迁移不能重新开启完整冷却。`P|` 的尾部字段包含 owner seat 与冲刺状态，用于拦截正常代码产生的 lane/seat 错配并重放推进。真人的心率/体力会影响赛果，因而 owner 端的本地调参必须在赛前通过大厅指纹门禁锁定，不能只信任上报后的最终心率。
...
- 联机 wire 格式虽然保持尾部追加兼容，但不同玩法协议版本不能混跑。大厅使用 `PV|<seat>|<version>|<tuningFingerprint>` 互相声明，所有人版本与完整调参指纹一致后才能准备/开始；`start` 消息也携带 `pv` 与 `tf` 作最终门禁。指纹由全部 `TUNING_GROUPS` 的稳定值计算，且登录大厅必须先完成与比赛场景共用的一次性调参加载，宁可拦住纯视觉调参差异，也不允许未来漏掉一个赛果参数。它是误配置防护，不是针对篡改客户端的反作弊。不要把版本或指纹塞进 32 字节上限的 `memberExtInfo`。
