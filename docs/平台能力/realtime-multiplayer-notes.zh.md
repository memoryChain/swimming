...
- **2026-09-18 玩法版本 48（当前）**：新增 200 米鲨鱼大乱斗。鲨鱼是关卡固定实体，在比赛第 15／35／55 秒进入警告与追击，轮次之间继续低速巡游并作为可碰撞障碍；每轮最多永久淘汰一名选手。房主独占目标选择、移动、咬合和淘汰判定，可靠帧事件发送 `sharkSequence＋targetLane`，房主 `S|` 快照追加鲨鱼位置、朝向、阶段、目标和永久淘汰掩码作为丢包恢复；新房主从最近快照继续模拟。最终 `R|` 同步淘汰及鲨鱼淘汰标记。47 及更旧版本不能混跑；离线回归覆盖快照／可靠事件编解码、固定三轮规则、模式存档与模型资源，双端真机表现待验证。
