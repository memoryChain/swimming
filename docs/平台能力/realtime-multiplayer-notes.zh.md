...
- **2026-09-12 玩法版本 28**：心率由实际手臂起划次数驱动，并在起划时锁定本划 PERFECT 边界，最窄保留 30%。真人仍由 owner 权威，AI 由房主权威校正心率与运动；AI 的最近起划历史由既有固定步继续推演，迁移不重置。可靠输入 `s<side>,<heartRate×100整数>` 追加动作心率，输入回放只临时覆盖这次起划，随后恢复更晚的 owner 状态；不能让迟到输入污染持续心率。恢复期间不划水即可降心率，踢腿和特殊动作不额外加减心率。普通状态快照的心率仍为整数，不移动既有字段。当时要求双方更新至 v28；当前要求见 v31；真机联调尚待验证。
