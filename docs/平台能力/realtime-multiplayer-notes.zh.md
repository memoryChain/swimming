...
| `net/NetSwimmerLook.ts` | 按开赛角色摘要解析模型、肤色与衣帽配色，和属性使用同一份快照。 |
...
- **2026-09-24 玩法版本 47**：Loading 等待完整参赛模型、动作、换色和附属资源，模型初始化按帧排队；完整阵容在赛前展示之前建立，共享随机数消费顺序随之调整。加载期间保持空输入帧上传，提前 GO 暂存到本机准备后执行。仍沿用原有超时兜底和房间保活，两端必须同版本，真机待验证，见[比赛加载就绪与分帧初始化](../比赛加载就绪与分帧初始化.zh.md)。

- **2026-09-24 玩法版本 46**：邀请进房等待存档就绪；真人外观与属性共用完整角色快照。角色广播及准备确认绑定成员标识，验证并冻结开赛名单，有限补发开赛包。进退房串行，处理超时、连续邀请和迟到回调；保活重赛沿用平台本人座位，不调用 endGame。单机及赛中同步规则保持原有路径。45 及更旧版本不得混跑；离线回归已通过，双端真机待验证，见[邀请进房与角色同步健壮性](../邀请进房与角色同步健壮性.zh.md)。
...
- **2026-09-13 玩法版本 41（当前）**：入口依次为标准竞速200米、狂野模式200米、狂野模式400米，单程50米；三个入口继续共用最高档AI，仅标准竞速入口关闭转向偏移，三个入口均保留碰撞及身体侧滚；更名不改变玩法协议。赛程由既有mode ID统一解析，准备页、房间、赛中、折返、完赛和结算共用赛程读取。房间切换只更新文本和选中状态，开赛/保活重开沿用已同步模式；无新增wire字段。40及更旧版本不得混跑，避免400米模式在旧端提前200米完赛。
