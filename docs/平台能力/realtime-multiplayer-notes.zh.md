...
- **2026-09-19 玩法版本 78（当前）**：统一娱乐模式在首位选手触壁后由房主锁定导演。未激活的预告立即取消，不再等待锚点或安排返场；已激活事件进入收尾阶段并完成自身必要结算，驻留漩涡、水雷和未拾取苏打继续保留到本局真正结束。锁定复用 `S|` 导演阶段字段，访客只应用房主快照，房主迁移后也保持关闭，不增加无线字段。版本 77 及更旧客户端不能混跑，避免终点后的访客继续播报或激活新事件。
