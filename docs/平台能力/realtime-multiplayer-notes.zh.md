...
- **2026-09-19 玩法版本 74（当前）**：巡游鲨鱼新增防卡死补咬。房主根据鲨鱼与所有选手（含远程真人权威位置副本）的持续接触、行进方向和赛程进度判断：逆向接触约 1.5 秒仍无法有效前进，或被反推 0.8 米时，复用现有可靠鲨鱼击倒事件进入急救复活；访客不运行本地纠缠判定。短暂擦碰、同向同行、正常前进、急救和无敌状态不触发，同一目标及鲨鱼全局均有冷却。无线格式不变，但房主玩法语义改变，因此版本 73 及更旧客户端不能混跑。
