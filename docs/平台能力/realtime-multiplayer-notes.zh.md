...
- **2026-09-18 玩法版本 62（当前）**：定时炸弹从仅击飞失速改为致命击倒后原进度重生，复用 2.5 秒急救、零速入水和 2 秒无敌状态；急救或无敌中的选手不会再次成为装雷或传雷目标。可靠 `b` 结算事件追加房主权威赛程距离，`S|` 恢复快照继续兜底丢包和房主迁移；不写入永久淘汰或 DNF。61 及更旧版本不能混跑。
