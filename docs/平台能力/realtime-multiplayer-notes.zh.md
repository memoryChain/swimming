...
- **2026-09-18 玩法版本 52（当前）**：原末位淘汰赛改为 200 米炮火逃生赛。房主随机选择目标并锁定预警落点，核心命中最多淘汰一人，外围只同步击飞；可靠 `l` 事件发送炮击编号／落点／预警时长，可靠 `x` 事件发送命中掩码／淘汰泳道。`S|` 快照追加炮击版本、淘汰和完成掩码、当前活动炮弹落点及剩余时间，支持丢包恢复和房主迁移；`R|` 单列炮击淘汰原因。51 及更旧版本不能混跑；离线回归覆盖命中分区、唯一淘汰、活动炮弹恢复、最低人数保护和事件／快照编解码，双端真机表现待验证。
