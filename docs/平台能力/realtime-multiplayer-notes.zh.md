...
- **condition 权威**：真人的体力比例/心率随 frame self（广播降级时随 `P|`/`IN|`）由 owner 提供，真正 AI 由房主 `S|` 提供。心率在网络上向下取整，版本 23 起仅作显示。正体力量化至少保留千分之一，避免远端提前进入耗尽推进；旧 AI 冷却字段保留为 0，体力不会在房主迁移时恢复。`P|` 在尾部携带 owner seat，用于拦截正常代码产生的 lane/seat 错配。
...
- **2026-09-12 玩法版本 23**：体力按实际划水结算固定扣除，默认每划 1 点，不再恢复、降频或按心率/冲刺改变成本。有体力时整划推进不衰减，耗尽后新开始的划水锁定 50% 推进倍率；按住预支、基础补足、质量奖励与超时推进同倍率，已在途动作保持预算。心率仅显示，PERFECT 宽度固定读取调参。真正 AI 使用实际结算计数，在每个网络固定步动作完成后扣费；远端真人不计入 AI 消耗，仍采用 owner condition。沿用原字段，正体力量化至少为千分之一以保留耗尽边界，AI 旧冷却字段固定为 0；22 及更旧客户端不能混跑。详见 [体力按次消耗设计](../体力按次消耗设计.zh.md)，真机双端体验待验证。
...
- **2026-09-12 玩法版本 21**：采用直线划水第一版手感参数，基础／质量推进 1.5／2.6，按住预算上限 0.8，推进持续比例 0.45、锐度 0.3，固定／线性／二次水阻 0.03／0.24／0.12。低速动作轮速从 1.5 提至 1.8，高速仍为 2；按住动作倍率 1，调试入口玩家轮速倍率 1。除调试入口倍率外均为共享参数，玩家、AI 和远端输入回放同步使用；体力、蓄气、转向模式、输入门槛与拒绝重叠动作规则保持原有机制，不新增网络字段。20 及更旧客户端不能混跑。回放条件及基线见 `docs/直线划水手感第一版.zh.md`，真机体验待验证。
...
- **为什么必须同步**：养成改的是玩家 balance（`maxSpeed/energyTotal/perfectComboMaxOvercap/strokeQualityAccel/diveMaxLaunchSpeed/`**`weight`**），全都**影响比赛结果**；`weight` 还进碰撞击退结算。若各端对同一泳者用不同 balance，本地预测和碰撞会算出不同结果。养成是**本地存档**，既不在共享 roster 里，也无法从 `avatarId` 推导 → 必须显式同步。
