...
- **2026-09-18 玩法版本 55**：兴奋剂大乱斗的权威拾取在二维身体胶囊与短路径扫掠前增加赛程距离门槛，避免 200 米折返中不同趟数映射到同一泳池坐标后，提前拾取尚未显示的后续道具或回头误拾取前序道具。距离门槛与物理判定共用 3 米网络校正上限；54 及更旧版本不能混跑。
...
- **2026-09-18 玩法版本 49**：兴奋剂大乱斗移除 14 米逐泳道保证波，只保留 35～185 米的七波公共争抢，共 21 瓶。房主拾取判定从角色根节点圆点改为沿前进方向延伸的二维身体胶囊，并扫掠上一位置到当前位置的短路径；高度继续不参与，超过 3 米的网络校正不沿整段补捡。可靠拾取事件与快照格式不变，但 itemId 赛程和结果判定已改变，因此升级协议阻止旧客户端混跑。48 及更旧版本不能混跑；离线回归覆盖 21 瓶赛程、身体胶囊、短路径扫掠和过长校正拒绝，双端真机表现待验证。
...
- **2026-09-17 玩法版本 47**：兴奋剂赛程改为 14 米处每条泳道各一瓶的保证体验波，之后在 35／60／85／110／135／160／185 米生成七波随机争抢，每波三瓶且连续两波不重复同一组泳道。拾取半径提高到 1.2 米；道具使用单网格、单材质、顶点色的低模能量瓶，显式 Prefab 加载失败时仅保留一个大方块兜底。刷新与公共拾取采用事件播报。拾取者的橙红脉冲发光由同一条权威拾取事件触发，不参与比赛结算；拾取掩码扩展到 29 位，协议升级以阻止旧赛程按相同 itemId 混跑。46 及更旧版本不能混跑；离线回归覆盖赛程确定性、泳道覆盖、29 位快照与模型 Prefab 引用，真机双端表现待验证。

- **2026-09-17 玩法版本 46**：新增兴奋剂大乱斗。房主／单机权威判定拾取，可靠帧通道发送 `itemId＋collectorLane＋revision`，房主快照附带拾取版本与 18 位已收集掩码作为恢复兜底；各端按同一种子生成道具位置。每瓶恢复自身上限 50% 体力并增加 30 心率，高心率放大转向冲量并降低阻尼。45 及更旧版本不能混跑。
