...
- **2026-09-18 玩法版本 69（当前）**：娱乐模式新增 400 米长局，房间规则、开赛原子消息与 `NetRaceSession` 同步 200／400 米距离；导演按距离确定三至四或五至六个不重复事件，周期 `S|` 快照的激活锚点从四槽扩展到六槽。客机继续只应用房主权威事件、快照和比赛距离；保活重赛复用会话但重新同步本局距离与种子。版本 68 及更旧版本不能混跑，避免客机按 200 米结束或截断事件表。
