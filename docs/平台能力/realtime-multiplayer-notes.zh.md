...
- **v96 固定名单与提前发令（2026-09-21）**：`start.roster` 采用 `[座位,头像ID,昵称]` 元组，与既有养成摘要共同创建独立成员快照；房主和访客都只从该快照生成本局会话，不再在入场时读取可能不同步的大厅名单。本局初始房主由比赛身份确定，后续再按赛中迁移规则接管。校验 2～8 个唯一合法座位、包含房主和本人、身份字段长度以及完整开赛消息 1536 字节预算；缺失或非法名单不入场、不确认。`GO|` 到达先记录发令，只有场景监听器已注册且本机已报告赛前准备完成才交付一次；加载先后、重复广播、7 秒兜底和销毁都不会重复启动或丢掉提前发令。该修复不改变原有发令等待时限。
...
- **2026-09-22 协议版本105**：尾迹起划入口立即拒绝旧房主任期及超过1.5米的位置跳变，客机AI预测额外核验来源的有效快照与水面资格，避免采样间隙或断流后继续领优惠。已经起划的成本不追溯修改，不增加网络字段；104及更旧版本不得混跑。真实网络控制器已覆盖跨通道重复不续期、700毫秒过期、离房迟到包和迁移拒绝旧主；实际双端微信联调仍待完成。性能与验证记录继续维护在[尾迹总说明](../尾迹跟游玩法与实施计划.zh.md)。

- **2026-09-22 协议版本104**：狂野／娱乐加入尾迹跟游，普通手划起手锁定成本，真人沿owner体力、AI沿房主体力校正。`S|`、`P|`与输入帧self的原完赛槽位改为一字符36进制：最低位为完赛，其余位编码水面资格及跟游来源，不增加包长；103及更旧设备不能混跑。跟游读取原始权威位置重建有限路径，超过700毫秒无新数据失效，禁止使用渲染插值节点；房主切换清理路径并重新确认，保留已结算体力。8泳道1536字节预算、三通道往返、重生与事件回归通过，iOS／Android弱网及画面待真机确认。详见[尾迹跟游玩法与实施计划](../尾迹跟游玩法与实施计划.zh.md)。

- **2026-09-22 协议版本 103**：已由比赛管理器记录触壁时间的选手，不得再被普通淘汰或离房改写资格；`onNetPlayerQuit` 对已确认完赛选手直接确认通知，保留时间与排名，避免待退出队列重复等待。否则已完赛人数仍留在分子、被淘汰者从有效人数移除，会在两人比赛中提前结束尚未完赛者的冲线窗口。未完赛退出与急救退出继续原路径；不新增无线字段，同房设备须统一至 v103。

- **2026-09-22 协议版本 102**：权威切换时，除旧位置与插值缓存外，同时清空 `_authoritativeEvents`、短期冲击补发缓存及其游标、待监听／代次就绪的入站队列。退位后再次接管不得把上一任期的未发命中重新加入可靠帧，也不得在降级广播中重发上一任期冲击；名单或 `L|` 先确认新房主时，旧来源队列不得占满容量而丢弃新权威早到事件。销毁也使用同一事件清理入口；比赛身份、收发序号、玩法代次与已应用的持久状态保持原规则。没有新增字段或广播，同房设备须统一至 v102。

- **2026-09-22 协议版本 101**：访客已经收到当前有效房主的最终成绩时，房主进入结算后停止位置广播不能触发静默误接管，也不因其他成员的竞争 `S|`／`L|` 改信来源；原房主更新的成绩仍可正常覆盖。`Q|` 或有效名单确认房主离开后，继续原有接管与成绩失效流程，不能把结果缓存变成永久权威锁。`R|` 每行追加 `quit` 布尔字段，由房主发送、访客覆盖本地退出标记，避免退出通知丢失后两端结算原因不同；旧七字段行仍可解码，非法新字段拒绝整包。无新增广播频率，八人成绩包仍低于 512 字节回归预算；同房设备须统一至 v101。

- **2026-09-22 协议版本 100**：正常房主不得因自身广播没有回声而采信低优先级竞争者，避免 `_isHost` 与当前权威座位矛盾；收到自身 `S|`／`L|` 回声直接忽略，退位后的旧回声不能重新取得权威或延后真正的静默接管。访客通过名单、`S|` 或 `L|` 更换房主时立即清掉旧来源位置、前一帧插值和采样时间，等待新来源位置包，不再继续消费旧权威目标。保留原优先级规则、静默阈值、分包去重及按来源序号，不改变无线字段；同房设备须统一更新至 v100。

- **2026-09-22 协议版本 99**：`S|` 头部原保留槽位 4 写入 `!` 加 36 进制整包序号，`L|` 头部追加同格式序号；同一次发送两包共用序号，接收端按来源、按包类型独立去重，避免先到的包丢掉另一种状态。房主仲裁另保留每个来源已见到的最大轮次，拒绝另一通道的迟到旧轮重新夺权；真正更新的快照仍遵循原优先级与静默迁移规则。拒绝非本局成员；旧包和重复包不刷新心跳、不改位置或体力、不触发玩法监听。序号不会因本机接管或退位而清零。八个较大的头部修订／计数值按需改为带 `!` 的 36 进制，量化精度和槽位不变，完整压力包继续遵守 1536 字节预算。双方须使用 v99。

- **2026-09-22 开赛边界补修**：房主开赛超时后变更赛制，访客一旦采信新规则就取消旧开赛身份、名单、摘要、补发和超时任务，要求重新准备；保留已经发起的平台请求和开始信号，最新合法 `start` 才能入场。零种子合法，入场资格改按有效比赛身份判断，会话原样使用房主种子，禁止用本机随机数替换零值。微信双机弱网、房主迁移和保活重赛仍待真机验收。

- **2026-09-22 玩法版本 98**：完整有效名单确认成员离开后，与 `Q|` 共用有界退出队列；场景尚未就绪时保留，倒计时／跳水／比赛阶段边缘补交，成功后去重。倒计时名册建立后即可处理退出，普通淘汰仍限正式比赛；离房后的输入、自位置与旧房主玩法事件不再采信。首个 `S|` 前的可靠玩法事件按本局开赛身份中的房主采信，监听和事件代次尚未就绪时继续缓存，保持原快照静默迁移时限。不开逐帧退出轮询，不增加无线字段；版本 97 及更旧客户端不能混跑。

- **2026-09-22 玩法版本 97**：联机选手退出并成功淘汰后，该泳道终止本局急救任务，拒绝迟到的击倒事件和恢复快照；重生入口同时检查比赛淘汰资格。退出状态不能被恢复计时器撤销，重开按既有 `reset()` 清除恢复禁用状态。沿用既有退出通知、名单和恢复快照，不增加无线字段；房主及访客均执行相同门禁，版本 96 及更旧客户端不能混跑。自动回归覆盖急救前、倒地、闪退、无敌期间退出、迟到快照、重复退出及重开；微信双机仍待验收。

- **2026-09-21 协议版本96（当前）**：开赛固定名单，成绩改为 `R|hostPos,sequence|entries`。仅采信本局当前有效房主；拒绝已离房、非成员、旧格式、同来源重复／乱序序号及不合法成绩行。房主重复补发同一结果保留序号，实际结果变化或再次接任后发布增加序号，退位后停止发送；权威切换清掉旧来源缓存，但保留各来源序号防回拨。成绩在首个同源位置快照之前到达时仍可保留。名次广播重发和 4 秒本地回退规则不变。双方须使用 v96；真机弱网和连续重赛仍需验收。

- **2026-09-21 玩法版本94**：垃圾恢复三种瓶型与泡沫餐盒、每波六件按2＋2＋2入水、18槽、16秒漂浮及5.5秒下沉；分组由既有spawnOrder及年龄恢复。房主独占生成与接触，保留v90～93的可靠事件去重、迟到补偿、迁移和raceId外壳。`L|`每槽21个量化整数改36进制，精度与字段数不变；含外壳的18槽标准压力包1300字节，全负横坐标及六位接触修订压力包1390字节。双方须更新到v94，v93及更旧客户端不能混跑。真机性能与弱网验收仍待完成。

- **2026-09-21 玩法版本 93**：保活房间的每次开赛由房主生成独立 `raceId`，通过 `start` 和 `NetRaceSession` 传入比赛；接管房主沿用该身份，重赛或开赛重试生成新身份。所有赛中广播（`S|`、`L|`、`P|`、`IN|`、`NB|`、`R|`、`Q|`、`CR|`、`GO|`）与可靠帧统一封装为 `G|raceId|payload`，接收端在修改房主、序号、代次或玩法状态前过滤异局／无身份消息，服务帧内逐 action 校验。房间握手仍用原有协议声明和赛制确认，不调用 `endGame`。销毁控制器同时取消倒计时及其监听；微信适配层不打印带外壳的高频广播。导演锚点／事件代次采用带 `!` 标记的 36 进制，完整八人快照连外壳仍需满足 1536 字节回归预算。炸弹接近与交接改为世界折返坐标，辅助短传按携带者赛段方向限制同向前方。双方须更新至 v93；真机连续重赛和弱网仍需验收。
- **2026-09-21 玩法版本 92**：炸弹完成账本、结果事件与携带者爆炸分别去重；炮火命中不再因下一发修订先到而丢失，`x` 事件尾部追加本发爆心横坐标（毫米整数）。两类结果复用事件元数据附带比赛模拟时钟，接收端仅补 3 秒内冲击；迟到效果不回拨下一轮状态或覆盖其镜头。广播降级时纳入既有 32 条／3 秒／每次最多 4 条的冲击重发缓存，`S|` 字段和预算不变。爆炸范围统一按场景折返坐标计算，双方须更新至 v92。

- **2026-09-21 玩法版本 91**：最终复查补齐五处边界遗漏。补给账本按真实领取顺序保存，避免苏打与冰沙逆序改变心率和冷静状态；`S|` 头部索引 45 改为 `!` 加 28 字符的紧凑账本，21 个领取序号各占一字节（道具编号×8＋泳道，255 表示缺序），缺序时等事件或完整账本后连续结算。迟到可靠帧仍处理独立权威效果，但不重放旧普通输入；监听尚未绑定的事件有界暂存并在绑定后恢复。垃圾接触先于对应代次槽位快照时也暂存，快照到达后只补一次效果。广播降级时，水雷冲击和硬垃圾接触保留最多 32 条、3 秒期限的恢复副本，随周期快照每次轮询补发最多 4 条；软垃圾轨迹仍由快照恢复，普通可靠同步不增加重发广播。连续断流超过窗口不补播旧冲量，有限重发不等于保证递送。v90 及更旧客户端不能混跑。

- **2026-09-21 玩法版本 90**：交付自检修复六项缺陷。跳水兜底排除娱乐击倒；补给回收与收益独立去重，`S|` 头部追加最多 21 位领取者账本（0 为未知，1～8 对应泳道），丢失领取事件也可从周期快照补结算。另追加炮火／炸弹／鲨鱼三个激活代次；相关可靠事件尾部携带 `~代次~`，旧轮丢弃，先到的新轮事件进入最多 128 条的暂存队列，等快照完成控制器重置再回放。导演拒绝旧快照时，同包子状态也拒绝；迁移后的房主沿用已恢复代次。垃圾／水雷接触效果独立于物体快照 revision 去重，事件尾部附玩法时钟，最多补偿 3 秒内未结算冲量；更新的物体轨迹与新一代水雷不回拨。急救每泳道整数以带 `!` 标记的 36 进制编码，不改变毫秒／厘米精度，含新增账本的极端八人快照仍遵守 1536 字节预算。v89 及更旧客户端不能混跑。修复另包括 400 米娱乐档案归一化、炸弹画中画目标姿态保活；自动回归通过情况见交付自检报告，微信真机验收仍待执行。

- **2026-09-21 玩法版本 89**：已开始投放的苏打与冰沙改为世界公共物品。赛程距离仍决定波次投放时机，完成落水后由房主直接按水池世界坐标、身体胶囊和短路径判定拾取，不再按选手趟数过滤；模型、光柱与 AI 目标同步改用物理方位。可靠拾取事件与 21 位恢复掩码不变；版本 88 及更旧客户端不能混跑。
- **2026-09-20 玩法版本 88**：心跳苏打赛程新增冷静冰沙类型。每个三波洗牌袋固定两波苏打、一波冰沙，同一波三个道具类型一致；各端继续由共享种子和相同赛程锚点确定类型，拾取可靠事件仍只传道具编号与拾取泳道。`S|`、`P|` 与帧同步 self 尾部追加量化后的冰沙剩余毫秒，供丢包、广播模式和房主迁移恢复三秒推进／转向状态；版本 87 及更旧客户端不能混跑。
- **2026-09-20 玩法版本 86**：六合一心跳苏打的动态赛程确定性避开每个折返墙前后 4 米，并曾限制只有同趟选手能拾取已落水瓶子；同趟限制已由版本 89 废止，折返墙安全距离和落水门槛继续保留。

- **2026-09-20 玩法版本 85**：巡游鲨鱼的防卡死补咬不再直接击倒后补播动画，而是进入独立 `PATROL_BITE` 状态：先停顿蓄势 0.09 秒，在闭合节点通过既有可靠鲨鱼事件击倒并短扑，结束后回到巡游且不推进正式追猎轮次。状态继续使用鲨鱼快照原字段，动画按序号去重；访客漏收蓄势快照时由可靠击倒事件补播，房主迁移按剩余时间恢复。版本 84 及更旧客户端不能混跑。
- **2026-09-20 玩法版本 84**：单独水雷玩法新增房主权威的三波补充，并同步波次与每个复用槽位的代次。
- **2026-09-20 玩法版本 83**：心跳苏打单瓶恢复从角色体力上限的 50% 降至 30%，心率负担从 30 提至 40；拾取后 4 秒内允许心率继续上升但禁止自然回落，重复拾取刷新滞留时间。可靠拾取事件在各端启动相同滞留，真人继续由 owner condition 权威校正，AI继续由房主快照校正，不增加无线字段；版本 82 及更旧客户端不能混跑，避免旧端提前降低心率并采用不同的转向失控强度。
- **2026-09-20 玩法版本 82**：六合一首位选手完赛并进入收尾时，若当前事件为定时炸弹，房主立即把尚未装载的轮次写入已完成掩码；已装载炸弹仍自然爆炸或拆弹，没有当前炸弹时待命提示立即隐藏，导演随后按收尾阶段结束。取消状态复用现有炸弹 revision、完成掩码和周期快照，访客及迁移后的新房主不会继续生成已取消轮次，不增加无线字段；版本 81 及更旧客户端不能混跑，避免旧房主继续等待或启动终点后的炸弹。
- **2026-09-19 玩法版本 81**：房主的定时炸弹传递优先消费人物碰撞求解器的稳定接触记录，保证已经触发击退／翻滚的真实碰撞不会因求解器先把两人分开而漏传；无接触记录时继续使用身体椭圆、跨帧扫掠和定向短传。仍复用既有可靠传递事件与快照，不增加无线字段；版本 80 及更旧客户端不能混跑，避免同一次人物碰撞在不同规则版本上产生不同归属。
- **2026-09-19 玩法版本 80**：鲨鱼、炮火、定时炸弹和水雷的统一击倒急救从 2.5 秒延长到 3.5 秒；本地压暗层改为一个、两个、三个点循环的“急救中”文字，并只在状态进入时启动轻微呼吸动效。零速重生和 2 秒无敌保持不变，恢复快照继续同步权威阶段与剩余时间，不增加无线字段；版本 79 及更旧客户端不能混跑，避免各端复活时机不一致。
- **2026-09-19 玩法版本 79**：定时炸弹改为优先发给附近存在可追目标的选手，扩大身体接触范围，缩短交接冷却，并在接棒保护期暂停附近 AI 的立即躲避；同速追赶时，房主还可对前方 `0.4～2.8` 米、横向相差不超过 `1.2` 米且持续对齐 `0.18` 秒的同一目标完成定向短传。短传至少跨两个权威采样点确认，候选计时不进快照，迁移后只会重新确认；装雷、传递和快照格式不变。版本 78 及更旧客户端不能混跑。
- **2026-09-19 玩法版本 78**：统一娱乐模式在首位选手触壁后由房主锁定导演。未激活的预告立即取消，不再等待锚点或安排返场；已激活事件进入收尾阶段并完成自身必要结算，驻留漩涡、水雷和未拾取苏打继续保留到本局真正结束。锁定复用 `S|` 导演阶段字段，访客只应用房主快照，房主迁移后也保持关闭，不增加无线字段。

- **2026-09-19 玩法版本 77**：漂浮水雷直接触雷改为“水雷自身椭圆＋人物身体椭圆”的扩张扫掠，保持原总静态范围不变；定时炸弹贴身传递改为双方身体椭圆的相对移动扫掠，覆盖单帧擦身而过并拒绝超过 3 米的网络校正补传。两项仍仅由房主判定，复用既有可靠 `i`／`t` 事件和周期快照，不增加无线字段；垃圾退场只影响隐藏单机调试模式的表现。版本 76 及更旧客户端不能混跑。

- **2026-09-19 玩法版本 76**：统一娱乐导演、炮火和水雷增加同修订乱序快照保护。导演与炮火的阶段剩余时间只能向前减少；水雷按最近一次权威漂移时钟拒绝倒序包，已爆障碍不能被旧存活掩码复活。无线字段未增加，但接收语义会影响事件切换和障碍存活，因此版本 75 及更旧客户端不能混跑。
- **2026-09-19 玩法版本 75**：动态激活的水雷新增出生安全保护。若水雷锚点落入任一在场选手周围的扩大安全椭圆，该雷先保持隐藏且不参与碰撞；房主仅在安全区连续清空后启用。`S|` 水雷原保留槽改为同步启用掩码，访客、晚加入与房主迁移据此恢复相同状态。六合一苏打继续生成在领先位置前方，垃圾与炮火继续使用各自的提前落水／预警规则，不套用水雷伤害半径。版本 74 及更旧客户端不能混跑。
- **2026-09-19 玩法版本 74**：巡游鲨鱼新增防卡死补咬。房主根据鲨鱼与所有选手（含远程真人权威位置副本）的持续接触、行进方向和赛程进度判断：逆向接触约 1.5 秒仍无法有效前进，或被反推 0.8 米时，复用现有可靠鲨鱼击倒事件进入急救复活；访客不运行本地纠缠判定。短暂擦碰、同向同行、正常前进、急救和无敌状态不触发，同一目标及鲨鱼全局均有冷却。无线格式不变，但房主玩法语义改变，因此版本 73 及更旧客户端不能混跑。
- **2026-09-19 玩法版本 73**：统一娱乐事件不再前段连续挤在一起。200 米按三／四事件赛程锚点分散，事件间至少留 5 秒；400 米按五／六事件锚点覆盖到约 90%，事件间至少留 8 秒。主事件表完成而比赛仍处于 `RACING` 时，房主只从炮火、鲨鱼、定时炸弹中安排不连续重复的安全返场，并先等待 10～12 秒。`S|` 导演快照尾部追加单调激活序号、最后激活事件、返场轮次和返场事件，访客据此识别同一种事件的再次激活；若客机错过整段返场，也能先按最后激活事件静默重置旧子状态，再消费本帧权威快照。导演状态在鲨鱼等子玩法状态之前应用。版本 72 及更旧客户端不能混跑。
- **2026-09-19 玩法版本 72**：漩涡单玩法和统一娱乐模式接入低概率超级漩涡。独立模式由比赛种子确定唯一中段超级规格；统一模式的房主导演在 `S|` 快照尾部追加 `specialMask`，访客、晚加入和新房主据此恢复同一规格。水流范围与力度会影响赛果，因此版本 71 及更旧客户端不能混跑；泳者结果仍沿用 owner／host 位置与姿态权威纠正，不增加逐帧事件。

- **2026-09-19 玩法版本 71**：统一娱乐模式新增可靠 `u` 击倒事件，携带全局恢复序号、泳道、原因与权威进度。访客在可靠帧到达时立即进入急救状态；周期 `S|` 恢复快照继续负责掉包、迟到加入和房主迁移兜底，不再是正常击倒的唯一入口。版本 70 及更旧版本不能混跑，避免访客只显示爆炸却延迟进入急救。

- **2026-09-18 玩法版本 70**：独立水雷和统一娱乐模式水雷改为一次性障碍；每枚水雷触爆后在本局永久失活，仅在重新比赛时随控制器重置恢复。房主继续通过可靠 `i` 事件判定命中，周期 `S|` 快照只同步权威漂移时钟与存活掩码，不再同步逐雷重生计时；原快照槽位保留为空的兼容占位，避免后续娱乐导演字段移位。版本 69 及更旧版本不能混跑，避免旧客户端在约 4.2 秒后错误复活水雷。

- **2026-09-18 玩法版本 69**：娱乐模式新增 400 米长局，房间规则、开赛原子消息与 `NetRaceSession` 同步 200／400 米距离；导演按距离确定三至四或五至六个不重复事件，周期 `S|` 快照的激活锚点从四槽扩展到六槽。客机继续只应用房主权威事件、快照和比赛距离；保活重赛复用会话但重新同步本局距离与种子。版本 68 及更旧版本不能混跑，避免客机按 200 米结束或截断事件表。

- **2026-09-18 玩法版本 68**：炮火外围击退椭圆扩大到纵向 3.4 米／横向 2.85 米，水雷外围爆炸扩大到 3.2 米／2.5 米，定时炸弹外围爆炸大幅扩大到 5.2 米／4.2 米。三者仍由房主计算受影响泳道掩码并通过既有可靠事件下发，未新增 wire 字段；同步放大现有池化水爆网格的变换，不增加节点、材质或逐帧几何。版本 67 及更旧版本不能混跑，避免范围参数不同。

- **2026-09-18 玩法版本 67**：统一娱乐模式取消事件结束广播和额外段间空档。当前强事件完成必要结算后，房主在同一次导演状态迁移中结束旧事件并直接进入下一事件的 5～6 秒预告；旧场景内容仍按驻留掩码继续生效，但任一时刻仍只有一个主动爆发事件。没有新增 wire 字段，周期快照继续同步阶段、事件序号和剩余时间；版本 66 及更旧版本不能混跑，避免各端使用不同衔接节奏。
...
- **2026-09-18 玩法版本 62**：定时炸弹从仅击飞失速改为致命击倒后原进度重生，当时复用 2.5 秒急救、零速入水和 2 秒无敌状态；急救或无敌中的选手不会再次成为装雷或传雷目标。可靠 `b` 结算事件追加房主权威赛程距离，`S|` 恢复快照继续兜底丢包和房主迁移；不写入永久淘汰或 DNF。61 及更旧版本不能混跑。

- **2026-09-18 玩法版本 61**：鲨鱼和炮火击倒后的急救等待从 1.5 秒延长到 2.5 秒；本地玩家在该阶段显示独占的压暗“急救中……”画面，重新入水后仍以零速度起步并获得 2 秒无敌。该条为历史版本记录，当前时长见版本 80。娱乐玩法统一赛事广播、个人反馈与持续状态的视觉通道；这些文字表现只消费既有权威事件和恢复快照，不增加网络字段。协议升级避免新旧客户端对恢复时刻产生分歧。
