...
- **2026-09-18 玩法版本 63（当前）**：水雷障碍在可靠 `i` 命中事件之外，把权威漂移时钟、存活掩码和逐雷重生剩余时间追加到 `S|` 周期快照。访客接受同 revision 的计时修复、拒绝更旧 revision；丢失命中事件、晚到快照和房主迁移后均能恢复水雷状态。水雷隐藏表现改为只在显隐边沿关闭节点，隐藏期间不再逐帧遍历。62 及更旧版本不能混跑。
