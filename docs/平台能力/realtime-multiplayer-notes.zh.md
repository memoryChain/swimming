...
- **2026-09-18 玩法版本 70（当前）**：独立水雷和统一娱乐模式水雷改为一次性障碍；每枚水雷触爆后在本局永久失活，仅在重新比赛时随控制器重置恢复。房主继续通过可靠 `i` 事件判定命中，周期 `S|` 快照只同步权威漂移时钟与存活掩码，不再同步逐雷重生计时；原快照槽位保留为空的兼容占位，避免后续娱乐导演字段移位。版本 69 及更旧版本不能混跑，避免旧客户端在约 4.2 秒后错误复活水雷。
