...
- **2026-09-18 玩法版本 67（当前）**：统一娱乐模式取消事件结束广播和额外段间空档。当前强事件完成必要结算后，房主在同一次导演状态迁移中结束旧事件并直接进入下一事件的 5～6 秒预告；旧场景内容仍按驻留掩码继续生效，但任一时刻仍只有一个主动爆发事件。没有新增 wire 字段，周期快照继续同步阶段、事件序号和剩余时间；版本 66 及更旧版本不能混跑，避免各端使用不同衔接节奏。

- **2026-09-18 玩法版本 66**：统一娱乐模式从固定三事件改为确定性随机三至四事件。三类仍各保底一个，房主事件表快照新增事件数量，并把激活赛程锚点固定扩为四槽；事件表最多打包四个 ID。四事件局采用 5 秒预告、1.5 秒间隔和 75% 爆发时长，房主仍权威推进阶段，访客只应用快照。版本 65 及更旧版本不能混跑。

- **2026-09-18 玩法版本 65**：炮火、定时炸弹和水雷统一采用直接命中者击倒急救、外围选手仅受冲击击退的两层爆炸语义。定时炸弹 `b` 事件追加量化横向爆心与受冲击泳道掩码，水雷 `i` 事件追加受冲击泳道掩码；房主决定受影响名单，访客不按本地漂移位置重算。独立水雷与六合一水雷统一为直接触雷击倒，不再保留模式条件分支。版本 64 及更旧版本不能混跑。

- **2026-09-18 玩法版本 64**：统一娱乐模式把三事件表、导演阶段、当前序号、量化剩余时间、已激活／驻留掩码和三个事件各自的激活赛程锚点追加到 `S|` 周期快照。导演快照先于六个子玩法状态应用，访客可在丢失开始广播或房主迁移后按驻留掩码补建控制器；六个子玩法继续沿用各自可靠事件和状态快照。版本 63 及更旧版本不能与统一导演语义混跑。
- **2026-09-18 玩法版本 63**：水雷障碍在可靠 `i` 命中事件之外，把权威漂移时钟、存活掩码和逐雷重生剩余时间追加到 `S|` 周期快照。访客接受同 revision 的计时修复、拒绝更旧 revision；丢失命中事件、晚到快照和房主迁移后均能恢复水雷状态。水雷隐藏表现改为只在显隐边沿关闭节点，隐藏期间不再逐帧遍历。62 及更旧版本不能混跑。

- **2026-09-18 玩法版本 62**：定时炸弹从仅击飞失速改为致命击倒后原进度重生，复用 2.5 秒急救、零速入水和 2 秒无敌状态；急救或无敌中的选手不会再次成为装雷或传雷目标。可靠 `b` 结算事件追加房主权威赛程距离，`S|` 恢复快照继续兜底丢包和房主迁移；不写入永久淘汰或 DNF。61 及更旧版本不能混跑。

- **2026-09-18 玩法版本 61**：鲨鱼和炮火击倒后的急救等待从 1.5 秒延长到 2.5 秒；本地玩家在该阶段显示独占的压暗“急救中……”画面，重新入水后仍以零速度起步并获得 2 秒无敌。娱乐玩法统一赛事广播、个人反馈与持续状态的视觉通道；这些文字表现只消费既有权威事件和恢复快照，不增加网络字段。协议升级避免新旧客户端对恢复时刻产生分歧。

- **2026-09-18 玩法版本 60**：鲨鱼和炮火核心命中不再永久淘汰。房主通过可靠事件同步击倒泳道与权威赛程距离；`S|` 快照追加每条泳道的击倒／无敌阶段、剩余时间、重生距离和版本，访客与接任房主据此恢复。泳者等待 1.5 秒后复用同一节点在原进度、本泳道中心以零速度重新入水，并获得 2 秒无敌；镜头目标不更换，最终结果不写入鲨鱼／炮击淘汰标记。协议升级阻止旧客户端继续执行永久淘汰语义。
- **2026-09-18 玩法版本 59**：漩涡外圈从无条件加速改为方向性水流：顺流侧获得切向推力和额外前进收益，逆流侧获得切向推力和额外反向阻力；外圈降低向心吸力，核心恢复完整吸力与回卷并适当收敛旋转强度。折返泳段按当前世界前进方向转换可见旋向，保证网格旋转与实际水流一致。公式继续由各端按同一随机布局确定性计算，不新增网络字段；协议升级阻止旧客户端按无方向外圈规则混跑。离线回归覆盖两个旋向、顺逆流两侧、入场／离场横推和折返世界旋向，双端真机手感待验证。

- **2026-09-18 玩法版本 58**：漩涡冲浪赛从固定坐标改为按房主比赛种子的独立随机流生成；四个泳段各一个，赛程距离和横向中心均限制在远离折返墙、出发端与池壁的安全带内，旋向继续交替。各端直接从同一种子生成相同布局，不新增网络消息，也不消耗 AI／阵容共享随机序列；漩涡表现改为提前展开并逐渐加速，通过后减速收缩，水流仍沿空间平滑曲线递增递减。57 及更旧版本不能混跑；离线回归覆盖种子复现、异种子差异和安全边界，双端真机表现待验证。

- **2026-09-18 玩法版本 57**：旧漂雷传递拆为两个独立入口。定时炸弹恢复房主按共享种子直接选择初始携带者，沿用可靠 `m`／`t`／`b` 事件与活动炸弹快照；水雷模式新增常驻障碍水雷，房主用扫掠碰撞判定触雷，并通过可靠 `i` 事件发送水雷 ID、命中泳道、爆炸位置和 revision。两个模式都不接管主镜头或事件画中画；旧 `mine-relay` 存档迁移到定时炸弹。56 及更旧版本不能混跑。
- **2026-09-18 玩法版本 56**：水雷接力赛曾提高漂浮追近速度与身体拾取范围，并在约 2.6 秒仍未真实接触时，由房主让水雷完成对当前目标的最后贴身吸附，避免整局没有一次装雷。该混合玩法已在 57 拆除。

- **2026-09-18 玩法版本 55**：心跳苏打大乱斗的权威拾取在二维身体胶囊与短路径扫掠前增加赛程距离门槛，避免 200 米折返中不同趟数映射到同一泳池坐标后，提前拾取尚未显示的后续道具或回头误拾取前序道具。距离门槛与物理判定共用 3 米网络校正上限；54 及更旧版本不能混跑。

- **2026-09-18 玩法版本 54**：水雷接力赛不再把每轮水雷直接挂到随机选手身上。水雷会提前在选手群前方从水下浮出，房主权威控制其柔和磁吸、目标切换和扫掠碰撞，真实接触后才启动完整引信；漂浮过久会平滑增强贴近速度。新增可靠 `n` 事件发送轮次及初始赛道坐标，原 `m` 改为碰撞拾取结果；`S|` 快照追加漂浮距离、横向位置、吸附目标和已漂浮时间，访客只插值展示。53 及更旧版本不能混跑；离线回归覆盖确定性生成、拾取前不计时、吸附、扫掠拾取、四类事件和漂浮／携带快照恢复，双端真机表现待验证。

- **2026-09-18 玩法版本 53**：新增 200 米水雷接力赛。全程单雷逐轮缩短引信，持雷者贴身自动传递，上一持有者有防回传保护，最后 0.8 秒锁定；爆炸只击飞失速，不淘汰，持雷冲线可拆弹。房主独占装雷、传递、爆炸／拆弹判定，可靠 `m`／`t`／`b` 事件发送持有关系与剩余引信；`S|` 快照追加活动轮次、持有者、保护计时、完成／爆炸掩码和每轮最终持有泳道压缩表，支持丢包恢复和房主迁移。52 及更旧版本不能混跑；离线回归覆盖传递保护、锁定、拆弹、确定性选择、事件与快照编解码，双端真机表现待验证。

- **2026-09-18 玩法版本 52**：原末位淘汰赛改为 200 米炮火逃生赛。房主随机选择目标并锁定预警落点，核心命中最多淘汰一人，外围只同步击飞；可靠 `l` 事件发送炮击编号／落点／预警时长，可靠 `x` 事件发送命中掩码／淘汰泳道。`S|` 快照追加炮击版本、淘汰和完成掩码、当前活动炮弹落点及剩余时间，支持丢包恢复和房主迁移；`R|` 单列炮击淘汰原因。51 及更旧版本不能混跑；离线回归覆盖命中分区、唯一淘汰、活动炮弹恢复、最低人数保护和事件／快照编解码，双端真机表现待验证。

- **2026-09-18 玩法版本 50**：新增 200 米漩涡冲浪赛。28／72／128／172 米固定生成交替旋向漩涡，外圈加速、核心回卷；AI 提前选择核心外路线。玩法不增加离散联机事件，继续依赖现有所有者／房主位置权威纠正，但固定赛程和水流语义改变，因此升级协议阻止旧客户端混跑。49 及更旧版本不能混跑；离线回归覆盖确定性水流、路线边界和快速比赛存档，双端真机表现待验证。

- **2026-09-18 玩法版本 49**：心跳苏打大乱斗移除 14 米逐泳道保证波，只保留 35～185 米的七波公共争抢，共 21 瓶。房主拾取判定从角色根节点圆点改为沿前进方向延伸的二维身体胶囊，并扫掠上一位置到当前位置的短路径；高度继续不参与，超过 3 米的网络校正不沿整段补捡。可靠拾取事件与快照格式不变，但 itemId 赛程和结果判定已改变，因此升级协议阻止旧客户端混跑。48 及更旧版本不能混跑；离线回归覆盖 21 瓶赛程、身体胶囊、短路径扫掠和过长校正拒绝，双端真机表现待验证。

- **2026-09-18 玩法版本 48**：新增 200 米鲨鱼大乱斗。鲨鱼是关卡固定实体，在比赛第 15／35／55 秒进入警告与追击，轮次之间继续低速巡游并作为可碰撞障碍；每轮最多永久淘汰一名选手。房主独占目标选择、移动、咬合和淘汰判定，可靠帧事件发送 `sharkSequence＋targetLane`，房主 `S|` 快照追加鲨鱼位置、朝向、阶段、目标和永久淘汰掩码作为丢包恢复；新房主从最近快照继续模拟。最终 `R|` 同步淘汰及鲨鱼淘汰标记。47 及更旧版本不能混跑；离线回归覆盖快照／可靠事件编解码、固定三轮规则、模式存档与模型资源，双端真机表现待验证。

- **2026-09-17 玩法版本 47**：心跳苏打赛程改为 14 米处每条泳道各一瓶的保证体验波，之后在 35／60／85／110／135／160／185 米生成七波随机争抢，每波三瓶且连续两波不重复同一组泳道。拾取半径提高到 1.2 米；道具使用单网格、单材质、顶点色的低模能量瓶，显式 Prefab 加载失败时仅保留一个大方块兜底。刷新与公共拾取采用事件播报。拾取者的橙红脉冲发光由同一条权威拾取事件触发，不参与比赛结算；拾取掩码扩展到 29 位，协议升级以阻止旧赛程按相同 itemId 混跑。46 及更旧版本不能混跑；离线回归覆盖赛程确定性、泳道覆盖、29 位快照与模型 Prefab 引用，真机双端表现待验证。

- **2026-09-17 玩法版本 46**：新增心跳苏打大乱斗。房主／单机权威判定拾取，可靠帧通道发送 `itemId＋collectorLane＋revision`，房主快照附带拾取版本与 18 位已收集掩码作为恢复兜底；各端按同一种子生成道具位置。每瓶恢复自身上限 50% 体力并增加 30 心率，高心率放大转向冲量并降低阻尼。45 及更旧版本不能混跑。

- **2026-09-13 玩法版本 45**：AI 使用角色＋等级共享属性解析，按完整泳道表抽选身份，避免本地玩家占位不同导致对手角色错配。体力快照仍传比例，各端用同一角色等级上限还原；智力只控制合法输入与策略，移除追赶补偿。新策略消费既有房主位置、速度、心率、蓄气、能力和体力校正，房主迁移不重置资源。远端真人继续隔离；无需扩展包字段，44及更旧版本不能混跑。离线回归已覆盖跨本地座位身份一致，双端真机待验收。

- **2026-09-13 玩法版本 44**：潜水哥与机甲均为固有禁海豚跳角色；水面、水下和上浮后都拒绝本地释放与远端回放。角色资格集中于 `CharacterAbilityState.supportsDolphin`，按钮沿用同源可用状态；拒绝不改变蓄气、体力与心率，无新增同步字段。43 及更旧版不能混跑。

- **2026-09-13 玩法版本 43**：潜水哥仅在短按松手确认后下潜，普通长按的按下踢腿反馈不产生深度。`k` 仍按原通道即时回放推进和动作，远端不以该事件启动能力；确认后的深度与保持时间由已有 owner frame self／P| 状态同步，AI 沿用房主权威。无新增字段或第二次踢腿事件，42 及更旧版不能混跑。短按去重、长按漏帧松手与实际实体 Y 已纳入回归，真机双端待验证。

- **2026-09-13 玩法版本 42**：接入角色固有能力。静态能力随现有角色 ID/等级 digest 解析；下潜深度、踢腿保持、连击层数和连击剩余时间追加到 S|／P|／frame self 尾部，真人跟随 owner，AI 跟随房主。复用序号去重，同一缓存快照只消费一次；房主迁移保留能力状态。机甲禁跳、潜水免泳者碰撞与风火轮加速规则跨端相同，41 及更旧版不能混跑。详情见[角色差异化能力](../角色差异化能力.zh.md)，真机双端待验证。
...
- **2026-09-12 玩法版本 28**：心率由实际手臂起划次数驱动，并在起划时锁定本划 PERFECT 边界，最窄保留 30%。真人仍由 owner 权威，AI 由房主权威校正心率与运动；AI 的最近起划历史由既有固定步继续推演，迁移不重置。可靠输入 `s<side>,<heartRate×100整数>` 追加动作心率，输入回放只临时覆盖这次起划，随后恢复更晚的 owner 状态；不能让迟到输入污染持续心率。恢复期间不划水即可降心率，踢腿和特殊动作不额外加减心率。普通状态快照的心率仍为整数，不移动既有字段。当时要求双方更新至 v28；当前要求见 v41；真机联调尚待验证。
...
- **2026-09-13 玩法版本 41**：入口依次为标准竞速200米、狂野模式200米、狂野模式400米，单程50米；三个入口继续共用最高档AI，仅标准竞速入口关闭转向偏移，三个入口均保留碰撞及身体侧滚；更名不改变玩法协议。赛程由既有mode ID统一解析，准备页、房间、赛中、折返、完赛和结算共用赛程读取。房间切换只更新文本和选中状态，开赛/保活重开沿用已同步模式；无新增wire字段。40及更旧版本不得混跑，避免400米模式在旧端提前200米完赛。

- **2026-09-13 玩法版本 40**：针对角色游速差异不足，初始技巧改为84～114，蛙妹90、忍者哥114。每级+1及技巧推进曲线保持不变，体力／爆发／体重不变。真人通过既有角色ID／等级digest得到新推进倍率，普通AI仍用共享基础值，无新增wire字段。39及更旧版本不得混跑。新增侧滚／自然心率／逐帧PERFECT及换手停顿对照，真机体验和双端联调待验证。
