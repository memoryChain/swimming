...
- **2026-09-20 玩法版本 83（当前）**：心跳苏打单瓶恢复从角色体力上限的 50% 降至 30%，心率负担从 30 提至 40；拾取后 4 秒内允许心率继续上升但禁止自然回落，重复拾取刷新滞留时间。可靠拾取事件在各端启动相同滞留，真人继续由 owner condition 权威校正，AI继续由房主快照校正，不增加无线字段；版本 82 及更旧客户端不能混跑，避免旧端提前降低心率并采用不同的转向失控强度。
