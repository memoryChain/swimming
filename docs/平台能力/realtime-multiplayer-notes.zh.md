...
- **2026-09-21 玩法版本 89（当前）**：已开始投放的苏打与冰沙改为世界公共物品。赛程距离仍决定波次投放时机，完成落水后由房主直接按水池世界坐标、身体胶囊和短路径判定拾取，不再按选手趟数过滤；模型、光柱与 AI 目标同步改用物理方位。可靠拾取事件与 21 位恢复掩码不变；版本 88 及更旧客户端不能混跑。
