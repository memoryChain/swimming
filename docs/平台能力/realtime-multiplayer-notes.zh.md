...
- **2026-09-18 玩法版本 59（当前）**：漩涡外圈从无条件加速改为方向性水流：顺流侧获得切向推力和额外前进收益，逆流侧获得切向推力和额外反向阻力；外圈降低向心吸力，核心恢复完整吸力与回卷并适当收敛旋转强度。折返泳段按当前世界前进方向转换可见旋向，保证网格旋转与实际水流一致。公式继续由各端按同一随机布局确定性计算，不新增网络字段；协议升级阻止旧客户端按无方向外圈规则混跑。离线回归覆盖两个旋向、顺逆流两侧、入场／离场横推和折返世界旋向，双端真机手感待验证。
