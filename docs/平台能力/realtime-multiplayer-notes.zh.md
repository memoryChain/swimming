...
- **2026-09-18 玩法版本 57（当前）**：旧漂雷传递拆为两个独立入口。定时炸弹恢复房主按共享种子直接选择初始携带者，沿用可靠 `m`／`t`／`b` 事件与活动炸弹快照；水雷模式新增常驻障碍水雷，房主用扫掠碰撞判定触雷，并通过可靠 `i` 事件发送水雷 ID、命中泳道、爆炸位置和 revision。两个模式都不接管主镜头或事件画中画；旧 `mine-relay` 存档迁移到定时炸弹。56 及更旧版本不能混跑。
