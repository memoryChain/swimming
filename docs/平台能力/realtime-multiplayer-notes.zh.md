...
- **2026-09-20 玩法版本 86（当前）**：六合一心跳苏打的动态赛程确定性避开每个折返墙前后 4 米；房主只允许已经完成落水、且与拾取者处于同一趟数的瓶子进入身体胶囊／短路径拾取，修复蹬壁翻转时吃到未显示苏打的问题。各端继续按同一种子和激活锚点生成位置，可靠拾取事件与快照字段不变；版本 85 及更旧客户端不能混跑。
