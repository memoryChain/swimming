...
- **condition 权威**：真人的体力比例/心率随 frame self（广播降级时随 `P|`/`IN|`）由 owner 提供，真正 AI 由房主 `S|` 提供。心率在网络上向下取整，确保 110/150/175 分区边界不会因四舍五入跨区；AI 的体力耗尽冷却剩余也随 `S|` 量化到毫秒，房主迁移不能重新开启完整冷却。`P|` 在尾部携带 owner seat，用于拦截正常代码产生的 lane/seat 错配。真人的心率/体力会影响赛果，因而 owner 端的本地调参必须在赛前通过大厅指纹门禁锁定，不能只信任上报后的最终心率。
...
- 联机 wire 格式虽然保持尾部追加兼容，但不同玩法协议版本不能混跑。大厅使用 `PV|<seat>|<version>|<tuningFingerprint>` 互相声明，所有人版本与完整调参指纹一致后才能准备/开始；`start` 消息也携带 `pv` 与 `tf` 作最终门禁。指纹由全部 `TUNING_GROUPS` 的稳定值计算，宁可拦住纯视觉调参差异，也不允许未来漏掉一个赛果参数。它是误配置防护，不是针对篡改客户端的反作弊。不要把版本或指纹塞进 32 字节上限的 `memberExtInfo`。
