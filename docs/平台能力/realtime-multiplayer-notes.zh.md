...
- **2026-09-21 玩法版本 92（当前）**：炸弹完成账本、结果事件与携带者爆炸分别去重；炮火命中不再因下一发修订先到而丢失，`x` 事件尾部追加本发爆心横坐标（毫米整数）。两类结果复用事件元数据附带比赛模拟时钟，接收端仅补 3 秒内冲击；迟到效果不回拨下一轮状态或覆盖其镜头。广播降级时纳入既有 32 条／3 秒／每次最多 4 条的冲击重发缓存，`S|` 字段和预算不变。爆炸范围统一按场景折返坐标计算，双方须更新至 v92。
