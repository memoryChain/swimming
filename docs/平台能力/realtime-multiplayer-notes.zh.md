...
- **2026-09-21 玩法版本 93**：保活房间的每次开赛由房主生成独立 `raceId`，通过 `start` 和 `NetRaceSession` 传入比赛；接管房主沿用该身份，重赛或开赛重试生成新身份。所有赛中广播（`S|`、`L|`、`P|`、`IN|`、`NB|`、`R|`、`Q|`、`CR|`、`GO|`）与可靠帧统一封装为 `G|raceId|payload`，接收端在修改房主、序号、代次或玩法状态前过滤异局／无身份消息，服务帧内逐 action 校验。房间握手仍用原有协议声明和赛制确认，不调用 `endGame`。销毁控制器同时取消倒计时及其监听；微信适配层不打印带外壳的高频广播。导演锚点／事件代次采用带 `!` 标记的 36 进制，完整八人快照连外壳仍需满足 1536 字节回归预算。双方须更新至 v93；真机连续重赛和弱网仍需验收。
