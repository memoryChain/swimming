...
`assets/resources/config/tuning.json` 当前为 v46。`values` 是稳定英文 ID 对应的数值，`updatedAt` 是保存时间，`version` 用于兼容性诊断和迁移提示；版本号本身不是选用哪个文件的优先级。
...
开发版单机比赛右上角提供“托管：关／开”，与“录制模式”“调参”并排。开启后，当前玩家从发令起跳到完赛都复用正式 AI 的输入控制器，仍走真实划水、体力、转向、碰撞、海豚跳和娱乐玩法规避逻辑，适合把注意力放在动作、美术与功能表现上。托管不会直接改速度，不会进入联机比赛；开启期间手动划水和技能输入被忽略，关闭后立即恢复。使用过托管的当局仍正常展示名次与完整结算，但不发放生涯奖励；下一局从头关闭托管即可恢复正常奖励。状态在当前应用会话及重赛间保留，重启应用复原为关闭。

“录制模式”仍只负责隐藏本机调试界面。先开启托管再进入录制模式时，托管继续运行，但其按钮会和调参、录制入口一起隐藏；它不自动点击结算页的“再来一次”。
...
单机调参会刷新需要重新解析的角色运动覆盖；联机沿既有会话隔离规则处理，不能在不同设备使用不同共享配置后期待相同结果。玩法规则变更需升级 `NetRaceProtocol.ts` 版本；本次为 v41。
...
`technique.speedGainPerPoint=0.003`：以84点为参考，标准PERFECT每10点约增加3%的基准稳定游速；设0关闭技巧差异。这个值经过校准曲线转换为完整手臂推进倍率，不直接乘最终速度。改变水阻、轮速或基础推进后，用`analyze-character-technique.cjs`重新测算。原角色质量推进及连击超速覆盖已移除，左右平衡不跟技巧成长。
