...
- `PoolsideProps_Merged`：救生站、泳具车、泳池梯、裁判席和长凳，共用一个池岸纯色 atlas；仰泳旗线已于 2026-09-10 移除。
...
按用户确认，源场馆与运行时均移除两组仰泳旗：24 片旗帜、2 根悬绳、4 根立杆、4 根斜撑，共 34 个源对象、424 triangles。此前旗帜配色说明仅作历史记录。剩余池岸道具 242 个源对象、4,680 triangles，仍合为 `PoolsideProps_Merged` 的一个 primitive。

复现入口：先运行 `remove-backstroke-flags.py` 编辑 editable，再加 `--sync` 在 master 重建池岸批次，随后执行本文标准 batch、dry-run、export 步骤。脚本允许在已移除旗线的源文件重复执行，不会额外删除其他对象。正式导出的池岸边界校验同步为 X 约 10..40m，地面与泳池梯接触检查保留。

`python3 scripts/check-venue-backstroke-removal.py <移除前GLB路径>` 验证节点与变换、33 个未修改网格、7 张内嵌图片逐字节不变，剩余道具几何不变且删除面只在两条旗线处。导出资源仍为 57 nodes、34 meshes、39 primitives；20,618 → 20,194 triangles，781,340 → 769,896 bytes。保留现有 GLB meta 与子资源 UUID，纹理策略检查通过。

Blender 5.2 导出器会优先用图片路径而非数据块名称作为 GLB 图片名。正式导出脚本在导出期间临时使用稳定的图片路径名，保留地板 JPEG、其余 PNG 及原打包字节，随后恢复源路径，避免临时 atlas 文件名或源稿文件名改变 Creator 子资源身份。没有启动或重启 Creator；引擎内观感与微信真机效果待现有编辑器重新导入后验证。
