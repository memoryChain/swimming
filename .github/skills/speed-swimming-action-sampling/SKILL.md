...
description: Retarget, validate, and sample imported FBX humanoid actions for this Cocos Creator swimming project. Use when adding Mixamo or other FBX motion clips, diagnosing mirrored or crossed arms, root-axis wobble, foot sliding or bad ground contact, generating sampled JSON curves, or assembling the multi-character debug-model preview.
---

# SpeedSwimming Action Sampling

Use this skill from this workspace whenever an external humanoid action is sampled onto the runtime swimmer.
...
3. Retarget with the existing project tool when it applies; otherwise add a focused tool under `tools/` before sampling. Identify rigs by required bone sets, not fragile object-name prefixes.
...
5. Sample every integer source frame, inclusive, with a project tool under `tools/`. Never compress every action to a fixed sample count.
6. Keep public ids and types in `assets/scripts/character/SampledActionMotionCurve.ts`, and write each emote's samples to `assets/race/sampled-actions/<action_id>.json`. Include one hip-translation sample for every rotation sample, update `SAMPLED_ACTION_IDS`, and let `SampledActionLoader` register the JSON asset.
...
- Never embed all action sample arrays in `SampledActionMotionCurve.ts`. Re-sampling one existing action should rewrite only its JSON file; update the public index only when the action roster or shared API changes.
