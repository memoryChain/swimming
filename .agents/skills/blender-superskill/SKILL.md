...
- 对已三角化或减面过的薄挡板去倒角时，优先保留源对象的局部轴、旋转和裁切端点；入口半板的最长边可能是斜向三角化边，不能把它当成盒体长轴。验证六个局部支承平面、正体积、正反广告 UV，再用原生 Join 重建原批次；导出后逐项对比未修改网格属性、索引和内嵌图片。

- 分层观众应按主镜头的实际可辨度分配模板预算：远层可用头身两块或单矩形，进入已有静态合批，避免每帧按距离切换网格或更新不可辨动作。矩形模板要关闭顶部随机偏移和宽度变形；核查普通看台与转角采用相同层级规则。
- 比较运行时生成几何时，旧版生成器必须配套加载同一提交的模板，不能复用工作区新模板。用真实场馆锚点验证全体落位与近层几何不变，并分别报告顶点、三角形、批次和原始缓冲预算；离线预览不能代替真机帧率验证。
