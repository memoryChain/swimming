...
## Triangulated Side-Tone Noise

Symptom:
- A low-poly seat, shell, or prop looks acceptable in its base colour, but a darker side-face overlay reveals large diagonal triangles from a side or distant view.
- Repeated instances turn the internal fan triangulation into a noisy stripe or sawtooth pattern.

Fix:
- Do not apply a strong colour contrast across a fan-triangulated concave or L-shaped side cap.
- Rebuild the visible side treatment as simple rectangular or convex planar bands whose triangulation cannot change the perceived silhouette.
- Keep the contrast subtle and inspect one repeated module from the actual gameplay/viewing distance before duplicating it.

Rule:
- Any face receiving a contrasting material or overlay is presentation geometry; its triangulation must be visually intentional.
