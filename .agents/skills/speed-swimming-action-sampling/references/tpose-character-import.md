...
- 前伸时主上臂、前臂与手的蒙皮矩阵相同，只能证明这些骨头的关系；还要检查实际顶点是否混入锁骨、脊柱或辅助骨影响。骨点共线不等于可见网格平直。
- 用绑定空间的肩肘长度、投影位置及离轴距离确定可重复的中央测量区，比较实际蒙皮与刚性随上臂的顶点位置；同时保留全肩轮廓检查，不能拿局部测量通过宣称整条手臂已完全贴合参考。
- 收窄胸肩权重时，按既有上臂权重和离轴距离平滑衰减，排除头颈与下肢影响区域；避免仅用外侧坐标误选衣摆。保留原关节索引、绑定矩阵、几何和贴图，只定点修改权重字节。
- 不要为了前伸单帧大幅旋转锁骨滚转、硬绑整个三角肌或同时收窄肘部权重。分别检查衣领和胸口、挥手／拍手、肘部拉伸，任一退化即排除候选；既有肘部阈值不能为了通过新模型而放宽。
- 候选必须可以从固定哈希基线复现，且避免二次执行重复衰减。本轮七角色的受限入口为 [repair-shoulder-weights.py](../scripts/repair-shoulder-weights.py)，它不是任意新模型的自动蒙皮工具。具体角色及验证范围见项目 `docs/角色自由泳肩臂蒙皮修正.zh.md`。
- 测试独立候选时，让所有网格检查读取同一候选目录，包括三角形边拉伸工具；不要让姿态工具读新 GLB、网格工具仍读运行时旧 GLB。检查 Creator 导入结果时，`library` 顶点数据中的关节仍是全骨架索引，不能再次通过 `jointMaps` 当成子集索引转换。
