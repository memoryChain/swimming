...
- 规范骨架的肘腕枢轴不一定落在可见手臂的中心线上。肩肘腕共线不等于网格伸直；独立对齐上臂、前臂和独立翻掌可能在肘部产生额外折弯。
- 如果角色绑定 T 姿势的手臂已伸直，前伸时优先保留其肘腕局部关系，整臂旋向目标，再整臂共同翻掌；与抱水、回臂姿势平滑混合。不同模型仍须分别检查可见外形。
- 数值验收使用实际 GLB 的 `worldMatrix * inverseBindMatrix`：直臂保持段的上臂、前臂、手应共享同一刚性蒙皮变换，保留绑定外形。必须同时检查俯视、侧视及保持段前后的动画，不能仅断言骨段夹角小于某个宽松阈值。
- 肘腕保持正确不代表肩部正确。前伸必须先让肩带随动作上举，再校正大臂；把肩带锁在 T 姿势横展方向、只旋转大臂，会形成宽 U 型和肩头折角。保留枢轴与骨长，不靠平移肩骨补外形。
- 双臂同时前伸是必查姿势：正视、俯视、侧视及斜视均检查肩头连接、手臂间距和肘腕；同时验证双臂同期与交替动作的完整周期。只看交替单臂会漏掉肩带协同问题。
- 细长手臂还要检查屈肘时的轴向扭转。独立追踪上臂方向、前臂方向和掌心，可能让肘部额外扭转接近半圈；骨长和目标方向全对，蒙皮仍会拧细、拉斜。自由泳先求前臂及掌心朝向，再用目标角色自己的前臂绑定旋转反求上臂滚转，以最短摆动恢复上臂方向，让肘部主要承担屈伸。不能用肌肉角色的外形替代细臂角色验收。
- 蒙皮形变检查必须包含辅助 `UpperarmTwist`／`ForearmTwist` 骨的实际权重。只比较三个主骨矩阵会漏掉关节连接处；应结合真实三角形边的形变统计、绑定长度／位置不变和多角度完整周期渲染。主骨连续、掌心正确也不能单独证明外形正确。
- 肩膀看似脱节时，要同时检查胸廓侧转幅度和相位。高肘回臂时胸肩应仍侧转，回臂侧肩膀随之抬起；如果出水时就达到峰值、回臂时过早放平，单独修手臂仍会显得别扭。将转角分布到胸背和上胸，并独立检查髋部和头部，避免只拧最上面一节或将整个人物一起大幅翻滚。前伸肩带的方向约束必须随胸廓转动，不能又锁回根节点水平面。
