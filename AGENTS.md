...
- 制作或修改 UI 动效、泳道及其他场景特效前，阅读 `tools/codex-skills/cocos-ui-asset-pipeline/references/motion-and-vfx.zh.md`。通用制作规范与踩坑维护在此；场景参数和验证记录留在对应 `docs/` 说明。
...
- 按用户确认，比赛 HUD 显示半透明手掌按钮及动态完美区弧线；左右半屏触控范围保留，不限制为按钮点击。显示只消费现有划水进度和松手判定，不更改玩法。
...
- **压缩贴图只有完整 mip 链才能启用 mip 采样。** 泳池常规纹理、角色 GLB 与换色遮罩按 `texture-mipmap-policy.js` 使用 `mipfilter: linear`；三个纯色色板保持 `none`。保留 `textureCompressConfig.genMipmaps=true`，由 Creator 离线生成各级 ASTC；`onAfterBuild` 必须校验引擎 CMIP 包装、逐级尺寸/块数据、完整 1×1 尾层及 PNG/JPG 回退，缺层应拒绝构建。其他 GLB 继续默认关闭 mip。禁止仅打开采样而沿用单层裸 `.astc`，否则 iOS 微信可能显示紫色/损坏贴图。重新导入后重建，并在 iOS/Android 真机检查角色 UV 接缝与换色遮罩。
