...
export class StrokeHeartRateModel {
    ...
    addBurden(amount: number, recoveryHoldSeconds = 0) {
        if (!Number.isFinite(amount) || amount <= 0) return;
        this.value = Math.min(180, this.value + amount);
        const holdSeconds = Number.isFinite(recoveryHoldSeconds)
            ? Math.max(0, recoveryHoldSeconds)
            : 0;
        if (holdSeconds > 0) {
            this.recoveryHoldUntil = Math.max(this.recoveryHoldUntil, this.clock + holdSeconds);
        }
    }
    ...
    tick(dt: number, freezeValue = false) {
        ...
        if (!freezeValue) this.approachInterval(Math.max(0, end - this.clock));
        this.clock = end;
    }
    ...
    private approachInterval(dt: number) {
        ...
        if (heldSeconds > 0) this.approach(heldSeconds, true);
        if (dt > heldSeconds) this.approach(dt - heldSeconds, false);
    }

    private approach(dt: number, suppressRecovery: boolean) {
        const target = this.targetHeartRate;
        if (suppressRecovery && target < this.value) return;
