...
// 28 paired ribbons are already baked in the mesh. Reveal half of them rather
// than creating more geometry, so the gather reads denser without a draw-call
...
export class DiveChargeGatherEffect {
    ...
    setActive(active: boolean) {
        if (this._requestedActive === active) return;
        this._requestedActive = active;
        this.syncVisibility();
    }

    releaseBurst(duration = RELEASE_BURST_SECONDS): number {
        if (!this._material?.isValid || !this._node.isValid) return 0;
    ...
    setCharge(intensity: number, progress: number) {
        ...
        if (this._intensity === next && this._progress === nextProgress) return;
        this._intensity = next;
        this._progress = nextProgress;
        if (!this._material?.isValid) return;
    ...
    setWorldPosition(position: Vec3) {
        if (!this._node.isValid || Vec3.squaredDistance(position, this._lastWorldPosition) < POSITION_EPSILON_SQ) return;
    ...
    destroy() {
        ...
        this._intensity = 0;
        if (this._node.isValid && this._node.active) this._node.active = false;
        if (this._material?.isValid) this._material.destroy();
        if (this._mesh?.isValid) this._mesh.destroy();
        this._material = null;
        this._mesh = null;
        if (this._node.isValid) this._node.destroy();
    }

    private loadEffect() {
        const token = ++this._loadToken;
        loadRaceAsset(RESOURCE_PATHS.diveChargeGatherEffect, EffectAsset, (error, effect) => {
            if (token !== this._loadToken || !this._node.isValid) return;
            if (error || !effect) {
    ...
    private build(effect: EffectAsset) {
        if (!this._node.isValid || this._material || this._mesh) return;
    ...
    private updateMotionParams(progress: number) {
        ...
        this._motionParams.set(phase, now, gatherSpeedForProgress(progress), GATHER_FIXED_DENSITY);
    }

    private syncVisibility() {
        const visible = this._releaseActive || (this._requestedActive && !!this._material?.isValid && this._intensity > 0);
        if (this._node.active !== visible) this._node.active = visible;
    }
}
...
function buildGatherGeometry() {
    ...
    // Each ray is authored as a canonical +X strip. The shader chooses a fresh
    // ring position/direction per ray cycle, so this mesh never exposes a fixed
    // set of visible emitter points around the swimmer.
    const direction = Vec3.RIGHT;
    const sideA = Vec3.UP;
    const sideB = Vec3.FORWARD;
    for (let rayIndex = 0; rayIndex < RAY_COUNT; rayIndex++) {
        // Density reveals the first N ribbons, so do not let their animation
        // phase also be sequential. A golden-ratio permutation staggers their
        // respawns instead of making the visible subset pulse in unison.
...
function appendReleaseHalo(positions: number[], normals: number[], uvs: number[], colors: number[], indices: number[]) {
...
function appendHaloRing(positions: number[], normals: number[], uvs: number[], colors: number[], indices: number[], axisA: readonly [number, number, number], axisB: readonly [number, number, number]) {
    for (let segment = 0; segment < RELEASE_HALO_SEGMENTS; segment++) {
        ...
        const direction0: readonly [number, number, number] = [axisA[0] * Math.cos(angle0) + axisB[0] * Math.sin(angle0), axisA[1] * Math.cos(angle0) + axisB[1] * Math.sin(angle0), axisA[2] * Math.cos(angle0) + axisB[2] * Math.sin(angle0)];
        const direction1: readonly [number, number, number] = [axisA[0] * Math.cos(angle1) + axisB[0] * Math.sin(angle1), axisA[1] * Math.cos(angle1) + axisB[1] * Math.sin(angle1), axisA[2] * Math.cos(angle1) + axisB[2] * Math.sin(angle1)];
...
function appendHaloVertex(positions: number[], normals: number[], uvs: number[], colors: number[], direction: readonly [number, number, number], radius: number, u: number, v: number) {
...
function appendRibbon(positions: number[], normals: number[], uvs: number[], colors: number[], indices: number[], direction: Vec3, side: Vec3, phase: number, densityRank: number, crossAxis: number) {
    const base = positions.length / 3;
...
function appendVertex(positions: number[], normals: number[], uvs: number[], colors: number[], direction: Vec3, side: Vec3, radius: number, width: number, phase: number, densityRank: number, crossAxis: number, u: number, v: number) {
    positions.push(direction.x * radius + side.x * width, direction.y * radius + side.y * width, direction.z * radius + side.z * width);
    ...
    // A=which of the two crossed ribbons this vertex belongs to.
    colors.push(densityRank, phase, 0, crossAxis);
}

function gatherSpeedForProgress(progress: number): number {
    const value = Math.max(0, Math.min(1, progress));
    if (value < 0.34) return lerp(2.2, 2.6, value / 0.34);
    if (value < 0.67) return lerp(2.6, 3.25, (value - 0.34) / 0.33);
    return lerp(3.25, 3.9, (value - 0.67) / 0.33);
}

function shaderTimeSeconds(): number { return director.root?.cumulativeTime ?? 0; }
function lerp(from: number, to: number, ratio: number): number { return from + (to - from) * Math.max(0, Math.min(1, ratio)); }
