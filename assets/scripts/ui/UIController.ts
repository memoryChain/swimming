...
export class UIController extends Component {
    ...
    onDestroy() { this.raceStartView?.destroy(); }
    ...
    showCountdown(value: number) {
        if (value !== 3 || value === this._lastStartCountdownValue) return;
    ...
    showGo() {
        this.raceStartView?.showGo();
        this.setSpeedBarVisible(false);
    }

    updateDiveCharge(power: number, visible: boolean) {
        this.raceStartView?.setCharge(power, visible);
    }
    ...
    showDiveCharging() {
        // READY 与中文操作提示在蓄力期间继续保留。
    }

    showDiveRelease(power: number, late: boolean) {
        this.raceStartView?.showRelease(power, late);
        this.updateDiveCharge(power, true);
    }

    showGliding() {
        this.raceStartView?.setCharge(0, false);
        this.setSpeedBarVisible(false);
    }
