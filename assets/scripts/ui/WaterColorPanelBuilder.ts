...
export class WaterColorPanelBuilder {
    ...
    build(parent: Node, w: number, h: number, camera: Camera | null = null): Node {
        const specs: SliderSpec[] = [
            ...
            { label: '水面色 R', min: 0, max: 255, integer: true, get: () => WATER_COLOR_TUNING.surfaceR, set: (v) => WATER_COLOR_TUNING.surfaceR = v },
            { label: '水面色 G', min: 0, max: 255, integer: true, get: () => WATER_COLOR_TUNING.surfaceG, set: (v) => WATER_COLOR_TUNING.surfaceG = v },
            { label: '水面色 B', min: 0, max: 255, integer: true, get: () => WATER_COLOR_TUNING.surfaceB, set: (v) => WATER_COLOR_TUNING.surfaceB = v },
            { label: '水面色浓度', min: 0, max: 1, get: () => WATER_COLOR_TUNING.surfaceStrength, set: (v) => WATER_COLOR_TUNING.surfaceStrength = v },
