...
export const TUNING_GROUPS: TuningGroup[] = [
    {
        ...
        controls: [
            ...
            control('brawl.curveHeading', '弯道幅度', '河道中段左右摆动的最大切线角度；修改后下次进入该模式生效。', () => RIVER_BRAWL_BALANCE.curveHeadingDegrees, (v) => RIVER_BRAWL_BALANCE.curveHeadingDegrees = v, 0.5, 0, 35, 1, '°'),
            ...
            control('brawl.bankSlowWidth', '岸边减速宽度', '身体进入河岸内侧这段距离后，水流推进和个人游速逐渐降低。', () => RIVER_BRAWL_BALANCE.bankSlowWidth, (v) => RIVER_BRAWL_BALANCE.bankSlowWidth = v, 0.1, 0.2, 4, 1, 'm'),
            control('brawl.bankFlowScale', '贴岸水流倍率', '身体贴住河岸时保留的河流推进比例。', () => RIVER_BRAWL_BALANCE.bankFlowSpeedScale, (v) => RIVER_BRAWL_BALANCE.bankFlowSpeedScale = v, 0.05, 0, 1, 2),
            control('brawl.bankPersonalScale', '贴岸个人倍率', '身体贴住河岸时保留的个人前进速度比例。', () => RIVER_BRAWL_BALANCE.bankPersonalSpeedScale, (v) => RIVER_BRAWL_BALANCE.bankPersonalSpeedScale = v, 0.05, 0, 1, 2),
            control('brawl.bankImpactRetention', '碰岸速度保留', '第一次撞到硬边界时保留的个人游速比例。', () => RIVER_BRAWL_BALANCE.bankImpactSpeedRetention, (v) => RIVER_BRAWL_BALANCE.bankImpactSpeedRetention = v, 0.05, 0, 1, 2),
            control('brawl.bankRecoverySeconds', '离岸恢复时间', '从最大岸边阻力恢复到清水速度所需的最长时间。', () => RIVER_BRAWL_BALANCE.bankRecoverySeconds, (v) => RIVER_BRAWL_BALANCE.bankRecoverySeconds = v, 0.05, 0, 2, 2, 's'),
            control('brawl.bankKnockbackGuardSeconds', '贴岸抗连踢时间', '碰岸后削弱继续朝岸外击退的持续时间，不提供无敌。', () => RIVER_BRAWL_BALANCE.bankKnockbackGuardSeconds, (v) => RIVER_BRAWL_BALANCE.bankKnockbackGuardSeconds = v, 0.05, 0, 2, 2, 's'),
            control('brawl.bankGuardImpulseScale', '贴岸击退倍率', '抗连踢期间继续朝岸外的击退保留比例。', () => RIVER_BRAWL_BALANCE.bankGuardOutwardImpulseScale, (v) => RIVER_BRAWL_BALANCE.bankGuardOutwardImpulseScale = v, 0.05, 0, 1, 2),
            ...
            control('brawl.aiBankRecoveryInset', 'AI离岸警戒', 'AI进入该岸边距离后优先向河道内侧脱离。', () => RIVER_BRAWL_BALANCE.aiBankRecoveryInset, (v) => RIVER_BRAWL_BALANCE.aiBankRecoveryInset = v, 0.1, 0.5, 4, 1, 'm'),
        ],
    },
    {
        name: '碰撞',
