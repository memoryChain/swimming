...
import { SharkArtPresentation } from './SharkArtPresentation';
...
import {
    EntertainmentEventBanner,
    type EntertainmentBannerCategory,
    type EntertainmentBannerIcon,
} from '../ui/SharkEventBanner';
...
function entertainmentBannerIcon(event: EntertainmentEventId): EntertainmentBannerIcon {
    switch (event) {
        case EntertainmentEventId.STIMULANT: return 'stimulant';
...
export class GameManager extends Component {
    ...
    private readonly _sharkArtPresentation = new SharkArtPresentation();
    private _sharkPresentationGeneration = 0;
    ...
    onDestroy() {
        ...
        this._sharkPresentationGeneration++;
        this._sharkArtPresentation.dispose();
    ...
    update(dt: number) {
        ...
        if (this._shark) {
            this._sharkLockOnOverlay.update(this._shark, dt);
        }
    ...
    private updateWhirlpoolBrawl(dt: number) {
        if (!isWhirlpoolBrawlMode() || this._modelDebugFlow?.active) {
            if (this._whirlpoolBrawl) {
                this._whirlpoolBrawl.dispose();
                this._whirlpoolBrawl = null;
            }
        ...
        if (!this._whirlpoolBrawl && this._worldRoot?.isValid) {
            const spawns = isEntertainmentBrawlMode()
        ...
        for (let i = 0; i < this._aiControllers.length; i++) {
            ...
            if (!controller || !swimmer || controller.remoteDriven) continue;
    ...
    private updateCannonBrawl(dt: number) {
        ...
        const launch = controller.currentLaunch();
        this._cannonBrawlPresentation?.update(
            dt,
            launch,
        ...
        for (let i = 0; i < this._aiControllers.length; i++) {
            ...
            ai.setCannonTargetZ(controller.targetZForAi(
                swimmer.distance,
                swimmer.node.position.z,
    ...
    private handleCannonLaunch(launch: CannonLaunch, broadcast: boolean) {
        ...
        if (broadcast && this._netRaceController?.isHost) {
            this._netRaceController.enqueueCannonLaunch(
                launch.strikeId, launch.targetDistance, launch.targetZ, launch.warningSeconds, launch.revision,
            );
        }
    }
    ...
    private applyExplosionShockwaveHit(lane: number, centerLateral: number) {
        ...
        swimmer.applyCollisionSoftnessImpulse(away * 1.35, -0.7);
        ...
        this._minefieldBrawl = new MinefieldBrawlController(
            ...
            isEntertainmentBrawlMode()
                ...
                ? {
                    courseX: 25,
                    lateral: 0,
                    alongRadius: WHIRLPOOL_BRAWL_TUNING.alongRadius
                        * WHIRLPOOL_SUPER_TUNING.alongRadiusScale + 1,
                    lateralRadius: WHIRLPOOL_BRAWL_TUNING.lateralRadius
    ...
    private setupLitterBrawl() {
        ...
        this._litterBrawl = new LitterBrawlController(
            ...
            COURSE_LAYOUT.poolWidth,
            lane => {
                const state = this._litterRacerStates[lane];
                const swimmer = this.swimmerForLane(lane);
                state.active = !!swimmer?.node?.active;
                state.finished = (this._raceManager?.hasSwimmerFinished(swimmer ?? null) ?? false)
                    || (swimmer?.distance ?? 0) >= getRaceDistance();
    ...
    private updateLitterBrawl(dt: number) {
        ...
        for (let lane = 0; lane < LANE_LAYOUT.laneCount; lane++) {
            const swimmer = this.swimmerForLane(lane);
            if (!swimmer) continue;
            swimmer.applyEnvironmentDrag(controller.environmentDragForLane(lane));
        }
        ...
        for (let index = 0; index < this._aiControllers.length; index++) {
            ...
            if (!ai || !swimmer || ai.remoteDriven) continue;
            ai.setLitterTargetZ(controller.targetZForAi(this.assignedLaneOfSwimmer(swimmer)));
        }
        this.activePlayerAutopilot()?.setLitterTargetZ(controller.targetZForAi(this._playerLaneIndex));
    }

    private handleLitterRigidImpact(impact: LitterRigidImpact) {
        const swimmer = this.swimmerForLane(impact.lane);
        if (!swimmer?.node?.active) return;
        swimmer.applyEnvironmentSpeedRetain(LITTER_BRAWL_TUNING.rigidSpeedRetain);
        swimmer.applyCollisionImpulse(
            -LITTER_BRAWL_TUNING.rigidBackwardImpulse,
    ...
    private setupSharkBrawl() {
        ...
        const fallback = this._sharkArtPresentation.buildFallback(visualRoot, SWIMMER_LAYER);
    ...
    private loadSharkArt(root: Node, visualRoot: Node, fallback: Node) {
        loadSwimmerPrefab((error, result) => {
            ...
            if (this._shark) this._sharkArtPresentation.bind(model, animation, this._shark);
            if (fallback.active) fallback.active = false;
            this._sharkArtPresentation.releaseFallback();
        }, RESOURCE_PATHS.sharkPrefabCandidates);
    }

    private syncSharkArtPresentation() {
        if (this._shark) this._sharkArtPresentation.sync(this._shark, 0, false);
    }
    ...
    private handleSharkStateChange(state: SharkState, previousState: SharkState) {
        if (state === SharkState.INACTIVE) {
            ...
            this._sharkPresentationGeneration++;
            this._sharkArtPresentation.reset();
            ...
            this._sharkLockOnOverlay.hide();
            // 退场只移动独立视觉根，不能让旧 Tween 改写权威节点或重开位置。
            this.syncSharkArtPresentation();
            if (!isEntertainmentBrawlMode()) {
                this._entertainmentEventBanner.enqueueEvent(
                    '玩具鲨收工啦 · 本场巡游结束',
    ...
    private applySharkKnockDown(lane: number, distance: number, revision: number, broadcast: boolean) {
        const swimmer = this.swimmerForLane(lane);
        const shark = this._shark;
        if (!swimmer || !shark || !Number.isFinite(revision)
            || revision < shark.sequence || revision <= this._lastSharkBitePresentationSequence) return;
        ...
        const generation = this._sharkPresentationGeneration;
        this.scheduleOnce(() => {
            if (generation !== this._sharkPresentationGeneration || this._shark !== shark
                || this._state !== GameState.RACING || !swimmer.node?.isValid) return;
            swimmer.cartoonRig?.triggerBigSplashAt(splashPosition, 3.1);
        }, SHARK_MODEL_PRESENTATION.biteSplashDelaySeconds);
        if (broadcast && this._netRaceController?.isHost) {
    ...
    private updateSharkBrawl(dt: number) {
        ...
        this._sharkArtPresentation.sync(shark, dt, !!this._netRaceController && !this._netRaceController.isHost);
    ...
    private buildUi(root: Node, w: number, h: number, done: (error?: unknown) => void) {
        ...
        raceUiBuilder.build(uiRoot, w, h, (error, refs) => {
            ...
            if (isEntertainmentBrawlMode() || isSharkBrawlMode()) {
                this._sharkLockOnOverlay.bind(this._worldRoot, COURSE_LAYOUT.waterY, SWIMMER_LAYER);
            }
            this.refreshSwimmerNameRoster();
    ...
    private handleSwimmerEliminated(swimmer: Swimmer) {
        ...
        const bitePresentationSeconds = this._shark?.state === SharkState.BITE
            ...
            : 0;
        const presentationSeconds = Math.max(bitePresentationSeconds, this._pendingPlayerEliminationDelay);
        this._pendingPlayerEliminationDelay = 0;
